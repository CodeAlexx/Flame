...
use flame_core::{ops_ext, DType, Device, Shape, Tensor};
use std::sync::Arc;

fn cuda_device() -> Arc<cudarc::driver::CudaDevice> {
    Device::cuda(0)
        .expect("CUDA GPU required. Set CUDA_HOME=/usr/local/cuda and export LD_LIBRARY_PATH=$CUDA_HOME/lib64:$LD_LIBRARY_PATH.")
        .cuda_device()
        .clone()
}
