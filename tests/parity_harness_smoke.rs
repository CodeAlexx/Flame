...
fn parity_harness_detects_match_drift_and_shape_mismatch() -> Result<()> {
    ...
    let a = Tensor::randn(Shape::from_dims(&[16, 32]), 0.0, 1.0, device.clone())?
        .to_dtype(DType::F32)?;
    let b = Tensor::randn(Shape::from_dims(&[8, 8]), 0.0, 0.5, device.clone())?
        .to_dtype(DType::F32)?;
    let c = Tensor::randn(Shape::from_dims(&[4, 4]), 0.0, 1.0, device.clone())?
        .to_dtype(DType::F32)?;
    ...
    let mut h = ParityHarness::load(&path, device.clone())?
        .with_tolerance(ParityTolerance { min_cos: 0.9999, max_abs_ratio: 0.01 });
    ...
    assert!(r_exact.cos > 0.999_99, "cos should be ~1; got {}", r_exact.cos);

    // Case 2: slight perturbation that exceeds 1% tolerance -> fails on
    // ratio (and likely on cos too at this magnitude).
    let noise = Tensor::randn(b.shape().clone(), 0.0, 0.5, device.clone())?
        .to_dtype(DType::F32)?;
    ...
    let wrong_shape = Tensor::zeros(Shape::from_dims(&[3, 3]), device.clone())?
        .to_dtype(DType::F32)?;
    let r_shape = h.compare("layer.shape_ref", &wrong_shape)?;
    assert!(!r_shape.passed);
    assert!(
        r_shape.note.as_deref().unwrap_or("").contains("shape mismatch"),
