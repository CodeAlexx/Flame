...
//!
//! 1. Contig bit-exact vs `bf16_ops::gelu_bf16`.
//! 2. Permuted view cos_sim ≥ 0.9999 vs materialized reference.
...
fn cuda_device() -> Arc<CudaDevice> {
    CudaDevice::new(0).expect("CUDA GPU required for tensor_iterator_gelu_parity")
}
...
#[test]
fn gelu_iter_contig_bit_exact() -> Result<()> {
    let dev = cuda_device();
    let x = make_bf16_tensor(dev, &[4096, 1024], 0xC0FFEE_43)?;
    assert!(x.is_contiguous());

    let ref_out = flame_core::bf16_ops::gelu_bf16(&x)?;
    let new_out = gelu_bf16_iter(&x)?;

    assert_bit_exact_bf16(&ref_out, &new_out, "gelu_iter contig")?;
    Ok(())
}

#[test]
fn gelu_iter_permuted_view_cos_sim() -> Result<()> {
    let dev = cuda_device();
    let h = 256usize;
    let w = 384usize;
    let x_contig = make_bf16_tensor(dev, &[h, w], 0xABCD_AAAA)?;

    let permuted = x_contig.as_strided(&[w, h], &[1, w], 0)?;
    assert!(!permuted.is_contiguous());

    let permuted_mat = permuted.contiguous()?;
    let ref_out = flame_core::bf16_ops::gelu_bf16(&permuted_mat)?;
    let new_out = gelu_bf16_iter(&permuted)?;
...
#[test]
fn gelu_iter_narrow_view_cos_sim() -> Result<()> {
    let dev = cuda_device();
    let src = make_bf16_tensor(dev, &[2, 32, 128], 0xDEAD_BEEF)?;
    ...
    let ref_out = flame_core::bf16_ops::gelu_bf16(&narrow_mat)?;
    let new_out = gelu_bf16_iter(&narrow_view)?;
