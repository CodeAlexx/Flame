...
fn adamw_bf16_params_fp32_states_match_fp32_reference() -> Result<()> {
    if cfg!(feature = "true_bf16_optimizer_states") {
        eprintln!(
            "skipping optimizer BF16 policy test because true_bf16_optimizer_states is enabled"
        );
        return Ok(());
    }
    if std::env::var("FLAME_SKIP_OPTIMIZER_COMPARE").is_ok() {
        eprintln!("skipping optimizer BF16 policy test by request");
        return Ok(());
    }
    ...
    for (a, b) in p_bf16_f32.iter().zip(p_ref_h.iter()) {
        assert_relative_eq!(a, b, max_relative = 1e-1, epsilon = 1e-5);
    }
...
fn sgd_bf16_params_fp32_states_match_fp32_reference() -> Result<()> {
    ...
    let p_bf16_f32 = param_bf16.tensor()?.to_vec_f32()?;
    for (a, b) in p_bf16_f32.iter().zip(p_ref_h.iter()) {
        assert_relative_eq!(a, b, max_relative = 5e-2, epsilon = 1e-5);
    }
