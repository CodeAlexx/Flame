...
fn main() -> Result<()> {
    ...
    // Test 1: U-Net style operations (residual connections + attention)
    {
        ...
        let attn_out = attn_out.permute(&[0, 2, 1, 3])?.reshape(&[b, h * w, c])?;
        ...
        if grads.contains(x.id())
            && grads.contains(conv1_weight.id())
            && grads.contains(conv2_weight.id())
        {
    ...
    // Test 2: Diffusion timestep embedding + noise prediction
    {
        ...
        // Noisy image
        let noisy_image =
            Tensor::randn(Shape::from_dims(&[4, 3, 64, 64]), 0.0, 1.0, device.clone())?
                .requires_grad_(true);
        ...
        let conv_weight =
            Tensor::randn(Shape::from_dims(&[64, 3, 7, 7]), 0.0, 0.1, device.clone())?
                .requires_grad_(true);
    ...
    // Test 3: Multi-scale feature extraction (like in diffusion encoders)
    {
        ...
        if grads.contains(input.id())
            && grads.contains(conv1_w.id())
            && grads.contains(conv2_w.id())
            && grads.contains(conv3_w.id())
        {
