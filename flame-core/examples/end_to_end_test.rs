...
fn main() -> Result<()> {
    ...
    println!("Test 1: Training a simple linear model");
    {
        ...
        let y_data = [5.0, 7.0, 9.0, 11.0, 13.0];
        ...
        for epoch in 0..100 {
            ...
            for i in 0..x_data.len() {
                ...
                // Forward pass
                let x =
                    Tensor::from_vec(vec![x_data[i]], Shape::from_dims(&[1, 1]), device.clone())?;
                let y_true =
                    Tensor::from_vec(vec![y_data[i]], Shape::from_dims(&[1, 1]), device.clone())?;
                ...
                if let (Some(w_grad), Some(b_grad)) = (grads.get(w_id), grads.get(b_id)) {
                    let mut updates = [(0, &mut w, w_grad), (1, &mut b, b_grad)];
                    optimizer.step(&mut updates)?;
        ...
        println!(
            "  Final: w={:.3}, b={:.3} (target: w=2.0, b=3.0)",
            w.to_vec()?[0],
            b.to_vec()?[0]
        );
    ...
    println!("\nTest 2: Conv2D operations");
    {
        ...
        // Create conv layer
        let conv = Conv2d::new(in_channels, out_channels, kernel_size, 1, 1, device.clone())?;
    ...
    println!("\nTest 3: Multi-layer network");
    {
        ...
        // Create layers
        let fc1 = Linear::new(input_dim, hidden_dim, true, &device)?;
        let fc2 = Linear::new(hidden_dim, output_dim, true, &device)?;
        ...
        // Check all gradients
        let has_all_grads = grads.get(input.id()).is_some()
