...
fn main() -> Result<()> {
    ...
    // Test 1: VAE Encoder operations (without Conv2D)
    {
        ...
        // Encoder layers
        let enc_w1 = Tensor::randn(
            Shape::from_dims(&[channels * height * width, 512]),
            0.0,
            0.01,
            device.clone(),
        )?
        ...
        let mu_w = Tensor::randn(
            Shape::from_dims(&[256, latent_dim]),
            0.0,
            0.1,
            device.clone(),
        )?
        .requires_grad_(true);
        let logvar_w = Tensor::randn(
            Shape::from_dims(&[256, latent_dim]),
            0.0,
            0.1,
            device.clone(),
    ...
    // Test 2: VAE Decoder operations
    {
        ...
        // Latent code
        let z = Tensor::randn(
            Shape::from_dims(&[batch, latent_dim]),
            0.0,
            1.0,
            device.clone(),
        )?
        ...
        // Decoder layers
        let dec_w1 = Tensor::randn(
            Shape::from_dims(&[latent_dim, 256]),
            0.0,
            0.1,
            device.clone(),
        )?
        ...
        let dec_w2 = Tensor::randn(Shape::from_dims(&[256, 512]), 0.0, 0.1, device.clone())?
            .requires_grad_(true);
        let dec_w3 = Tensor::randn(
            Shape::from_dims(&[512, output_dim]),
            0.0,
            0.1,
            device.clone(),
        )?
        ...
        if grads.contains(z.id())
            && grads.contains(dec_w1.id())
            && grads.contains(dec_w2.id())
            && grads.contains(dec_w3.id())
        {
    ...
    // Test 3: Complete VAE forward-backward (simplified)
    {
        ...
        // Input
        let x = Tensor::randn(
            Shape::from_dims(&[batch, input_dim]),
            0.0,
            1.0,
            device.clone(),
        )?
        ...
        .requires_grad_(true);
        let mu_w = Tensor::randn(
            Shape::from_dims(&[hidden_dim, latent_dim]),
            0.0,
            0.1,
            device.clone(),
        )?
        ...
        // Decoder
        let dec_w1 = Tensor::randn(
            Shape::from_dims(&[latent_dim, hidden_dim]),
            0.0,
            0.1,
            device.clone(),
        )?
        .requires_grad_(true);
        let dec_w2 = Tensor::randn(
            Shape::from_dims(&[hidden_dim, input_dim]),
            0.0,
            0.1,
            device.clone(),
        )?
        ...
        let kl_loss = mu
            ...
            .sum()?
            .mul_scalar(0.5)?;
        let total_loss = recon_loss.add(&kl_loss)?.div(&Tensor::from_slice(
            &[batch as f32],
            Shape::from_dims(&[]),
            device.clone(),
        )?)?;
    ...
    // Test 4: Hierarchical VAE (multiple latent layers)
    {
        ...
        // First latent layer
        let enc1_w = Tensor::randn(
            Shape::from_dims(&[input_dim, 128]),
            0.0,
            0.1,
            device.clone(),
        )?
        ...
        if grads.contains(x.id())
            && grads.contains(enc1_w.id())
            && grads.contains(enc2_w.id())
            && grads.contains(mu2_w.id())
        {
