...
fn main() -> Result<()> {
    ...
    // Test 1: Simple 2D mean
    {
        ...
        for (i, (&actual, &expected)) in mean_dim0_values
            .iter()
            ...
            .enumerate()
        {
            assert!(
                ...
                "Mean dim 0 mismatch at index {}: got {}, expected {}",
                i,
                actual,
                expected
            );
        ...
        for (i, (&actual, &expected)) in mean_dim1_values
            .iter()
            ...
            .enumerate()
        {
            assert!(
                (actual - expected).abs() < 1e-5,
                "Mean dim 1 mismatch at index {}: got {}, expected {}",
                i,
                actual,
                expected
            );
    ...
    // Test 2: Keepdim test
    {
        ...
        println!(
            "  Mean shape (keepdim=false): {:?}",
            mean_no_keepdim.shape().dims()
        );
    ...
    // Test 3: Multiple dimensions
    {
        ...
        let tensor = Tensor::randn(Shape::from_dims(&[2, 3, 4, 5]), 0.0, 1.0, device.clone())?;
