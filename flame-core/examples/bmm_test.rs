...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Manual computation check: expected={:.6}, actual={:.6}, diff={:.6}",
    ...
    let a2d = Tensor::randn(Shape::from_dims(&[3, 4]), 0.0, 1.0, device.clone())?;

    let b2d = Tensor::randn(Shape::from_dims(&[4, 5]), 0.0, 1.0, device.clone())?;

    let c2d = a2d.bmm(&b2d)?;
    println!(
        "2D BMM result shape: {:?} (expected [3, 5])",
        c2d.shape().dims()
    );
    ...
    match a_broadcast.bmm(&b_broadcast) {
        Ok(result) => {
            println!(
                "Broadcasting BMM succeeded! Shape: {:?}",
                result.shape().dims()
            );
        }
        Err(e) => {
    ...
    // Performance test
    println!("\n=== Performance Test ===");
    let large_a = Tensor::randn(Shape::from_dims(&[32, 64, 128]), 0.0, 1.0, device.clone())?;

    let large_b = Tensor::randn(Shape::from_dims(&[32, 128, 256]), 0.0, 1.0, device)?;
    ...
    println!(
        "Time for 10 BMM operations (32 batches of 64x128 @ 128x256): {:?}",
        elapsed
    );
