...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Output statistics - Mean: {:.6}, Variance: {:.6}",
        mean, variance
    );
    ...
    let input_3d = Tensor::randn(Shape::from_dims(&[10, 20, 64]), 0.0, 1.0, device.clone())?;
    ...
    for i in 0..num_samples {
        ...
        let var =
            sample.iter().map(|&x| (x - mean) * (x - mean)).sum::<f32>() / feature_size as f32;
    ...
    println!(
        "Average mean: {:.6}, Average variance: {:.6}",
        avg_mean, avg_var
    );
    println!("(Should be close to 0 and 1 respectively)");
