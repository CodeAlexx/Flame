...
fn main() -> Result<()> {
    ...
    // Test: Simple linear function gradient
    {
        ...
        // Parameters
        let w = Tensor::from_vec(vec![2.0], Shape::from_dims(&[1, 1]), device.clone())?
            .requires_grad_(true);
        let b = Tensor::from_vec(vec![3.0], Shape::from_dims(&[1]), device.clone())?
            .requires_grad_(true);
    ...
    // Test MSE gradient
    {
        ...
        let y_pred = Tensor::from_vec(vec![5.0], Shape::from_dims(&[1]), device.clone())?
            .requires_grad_(true);
