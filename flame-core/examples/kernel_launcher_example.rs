...
use flame_core::kernel_launcher::{templates, KernelLauncher};
...
fn example_custom_kernel(device: &Arc<CudaDevice>) -> Result<()> {
    ...
    let kernel_code = templates::elementwise_unary("polynomial_kernel", "x * x + 3.0f * x + 2.0f");
    ...
    let mut output =
        unsafe { device.alloc::<f32>(numel) }.map_err(|_| flame_core::FlameError::CudaDriver)?;

    let _ = launcher.prepare_kernel("polynomial_kernel", &kernel_code)?;

    let f = device
        .get_func("polynomial_kernel", "polynomial_kernel")
    ...
    unsafe {
        f.launch(config, (&mut output, &*input.data(), numel as i32))?;
    }
...
extern "C" __global__ void weighted_sum_kernel(
    ...
    let f = device
        .get_func("weighted_sum_kernel", "weighted_sum_kernel")
    ...
    unsafe {
        f.launch(
            config,
            (&mut output, &*a.data(), &*b.data(), params, numel as i32),
        )?;
    }
