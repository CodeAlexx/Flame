...
fn main() -> Result<()> {
    ...
    let x = Tensor::randn(Shape::from_dims(&[1]), 2.0, 0.0, device.clone())?
        .requires_grad_(true);
    ...
    let a = Tensor::randn(Shape::from_dims(&[2, 2]), 1.0, 0.0, device.clone())?
        .requires_grad_(true);
    let b = Tensor::randn(Shape::from_dims(&[2, 2]), 1.0, 0.0, device.clone())?
        .requires_grad_(true);
    let c = a.add(&b)?;
    let loss = c.sum()?;
