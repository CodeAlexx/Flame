...
fn main() -> Result<()> {
    ...
    // Test 1: Simple neural network training loop
    {
        ...
        let w1 = Tensor::randn(Shape::from_dims(&[input_dim, hidden_dim]), 0.0, 0.1, device.clone())?
            .requires_grad_(true);
        let w2 = Tensor::randn(Shape::from_dims(&[hidden_dim, output_dim]), 0.0, 0.1, device.clone())?
            .requires_grad_(true);
        ...
        for epoch in 0..num_epochs {
            ...
            if let (Some(w1_grad), Some(w2_grad)) = 
                (grads.get(w1.id()), grads.get(w2.id())) {
    ...
    // Test 2: Multi-step training with gradient accumulation
    {
        ...
        // Simple linear model
        let w = Tensor::randn(Shape::from_dims(&[input_dim, output_dim]), 0.0, 0.1, device.clone())?
            .requires_grad_(true);
        ...
        println!("   Average loss over {} steps: {:.4}", accumulation_steps, 
    ...
    // Test 3: Training with multiple losses (like in GANs or multi-task learning)
    {
        ...
        // Generator network
        let g_w = Tensor::randn(Shape::from_dims(&[latent_dim, data_dim]), 0.0, 0.1, device.clone())?
            .requires_grad_(true);
        ...
        // Generate fake data
        let z = Tensor::randn(Shape::from_dims(&[batch_size, latent_dim]), 0.0, 1.0, device.clone())?;
        let fake_data = z.matmul(&g_w)?;
        
        // Real data
        let real_data = Tensor::randn(Shape::from_dims(&[batch_size, data_dim]), 0.0, 1.0, device.clone())?;
    ...
    // Test 4: Complex model with skip connections (ResNet-style)
    {
        ...
        // Input
        let x = Tensor::randn(Shape::from_dims(&[batch_size, dim]), 0.0, 1.0, device.clone())?
            .requires_grad_(true);
        ...
        if grads.contains(x.id()) && grads.contains(w1.id()) && 
           grads.contains(w2.id()) && grads.contains(w3.id()) {
    ...
    // Test 5: Training with dynamic computation graph
    {
        ...
        for i in 0..3 {
            ...
            println!("   Iteration {}: Used path {}, got {} gradients", 
                     i, if i % 2 == 0 { "double matmul" } else { "matmul+relu+square" }, 
                     grads.len());
        }
