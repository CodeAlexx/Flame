...
fn main() -> Result<()> {
    ...
    let w1 = Tensor::randn(Shape::from_dims(&[128, 3, 3, 3]), 0.0, 0.02, device.cuda_device_arc())?;
    x = x.conv2d(&w1, None, 1, 1)?.silu()?;
    
    // Layer 2: 128 -> 128 channels, stride 2 (downsample)
    let w2 = Tensor::randn(Shape::from_dims(&[128, 128, 3, 3]), 0.0, 0.02, device.cuda_device_arc())?;
    x = x.conv2d(&w2, None, 2, 1)?.silu()?;
    
    // Layer 3: 128 -> 256 channels
    let w3 = Tensor::randn(Shape::from_dims(&[256, 128, 3, 3]), 0.0, 0.02, device.cuda_device_arc())?;
    x = x.conv2d(&w3, None, 1, 1)?.silu()?;
    
    // Layer 4: 256 -> 256 channels, stride 2 (downsample)
    let w4 = Tensor::randn(Shape::from_dims(&[256, 256, 3, 3]), 0.0, 0.02, device.cuda_device_arc())?;
    x = x.conv2d(&w4, None, 2, 1)?.silu()?;
    
    // Layer 5: 256 -> 512 channels
    let w5 = Tensor::randn(Shape::from_dims(&[512, 256, 3, 3]), 0.0, 0.02, device.cuda_device_arc())?;
    x = x.conv2d(&w5, None, 1, 1)?.silu()?;
    
    // Layer 6: 512 -> 512 channels, stride 2 (downsample)
    let w6 = Tensor::randn(Shape::from_dims(&[512, 512, 3, 3]), 0.0, 0.02, device.cuda_device_arc())?;
    x = x.conv2d(&w6, None, 2, 1)?.silu()?;
    
    // Layer 7: 512 -> 512 channels
    let w7 = Tensor::randn(Shape::from_dims(&[512, 512, 3, 3]), 0.0, 0.02, device.cuda_device_arc())?;
    x = x.conv2d(&w7, None, 1, 1)?.silu()?;
    
    // Layer 8: 512 -> 8 channels (latent)
    let w8 = Tensor::randn(Shape::from_dims(&[8, 512, 3, 3]), 0.0, 0.02, device.cuda_device_arc())?;
