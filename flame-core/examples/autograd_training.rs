...
use flame_core::{autograd_ops::BackwardOps, CudaDevice, Shape, Tensor};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Initialize weights
    let mut w1 =
        Tensor::randn(Shape::from_dims(&[2, 4]), 0.0, 0.5, device.clone())?.requires_grad_(true);
    let mut b1 = Tensor::zeros(Shape::from_dims(&[1, 4]), device.clone())?.requires_grad_(true);

    let mut w2 =
        Tensor::randn(Shape::from_dims(&[4, 1]), 0.0, 0.5, device.clone())?.requires_grad_(true);
    let mut b2 = Tensor::zeros(Shape::from_dims(&[1, 1]), device.clone())?.requires_grad_(true);
