...
use flame_core::autograd_v2::{backward, clear_tape, tracked_ops, TensorData};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "\nx (requires_grad=true) gradient: {:?}",
        x_grad.grad().map(|g| g.to_vec().unwrap())
    );
    println!(
        "y (requires_grad=false) gradient: {:?}",
        y_no_grad.grad().map(|g| g.to_vec().unwrap())
    );
