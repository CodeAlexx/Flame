use flame_core::{Tensor, CudaDevice, Shape, DType};
use flame_core::mixed_precision::{AMPContext, MixedPrecisionTensor, HalfTensor, GradScaler};
use std::sync::Arc;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bf16_max_diff = original_data.iter()
        .zip(bf16_data.iter())
        .map(|(a, b)| (a - b).abs())
        .max_by(|a, b| a.partial_cmp(b).unwrap())
        .unwrap();
    ...
    let mut weight1 = Tensor::randn(Shape::from_dims(&[512, 1024]), 0.0, 0.02, device.clone())?.requires_grad_(true);
    let mut weight2 = Tensor::randn(Shape::from_dims(&[1024, 512]), 0.0, 0.02, device.clone())?.requires_grad_(true);
