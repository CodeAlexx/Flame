...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let norms_before: Vec<f32> = layer_grads
        .iter()
    ...
    for (step, &norm) in norm_sequence.iter().enumerate() {
        ...
        if tracker.is_exploding(10.0) {
            println!(
                "Step {}: Gradient explosion detected! Norm = {}",
                step, norm
            );
        } else if tracker.is_vanishing(0.01) {
            println!(
                "Step {}: Gradient vanishing detected! Norm = {}",
                step, norm
            );
        } else {
    ...
    println!(
        "Time for 10 gradient clipping operations on 10x[1024x1024] tensors: {:?}",
        elapsed
    );
