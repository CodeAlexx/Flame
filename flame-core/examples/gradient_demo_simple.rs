...
fn main() -> Result<()> {
    ...
    let w = Tensor::randn(Shape::from_dims(&[8, 4]), 0.1, 0.0, device.clone())?
        .requires_grad_(true);
    ...
    if let Some(w_grad) = gradients.get(w.id()) {
        ...
        let clipped_norm = clipped_tensor.to_vec()?.iter().map(|g| g * g).sum::<f32>().sqrt();
        ...
        let normalized_norm = normalized.to_vec()?.iter().map(|g| g * g).sum::<f32>().sqrt();
