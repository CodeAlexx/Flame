...
fn main() -> Result<()> {
    ...
    // Test 1: Attention-style operations (common in diffusion models)
    {
        ...
        // Input sequence
        let x = Tensor::randn(
            Shape::from_dims(&[batch, seq_len, hidden_dim]),
            0.0,
            1.0,
            device.clone(),
        )?
        ...
        let scaled_scores = attn_scores.div(&Tensor::full(
            attn_scores.shape().clone(),
            scale,
            device.clone(),
        )?)?;
        ...
        if grads.contains(x.id())
            && grads.contains(w_q.id())
            && grads.contains(w_k.id())
            && grads.contains(w_v.id())
        {
    ...
    // Test 2: Timestep embedding operations (critical for diffusion)
    {
        ...
        let w1 = Tensor::randn(Shape::from_dims(&[1, embed_dim]), 0.0, 0.1, device.clone())?
            .requires_grad_(true);
        let w2 = Tensor::randn(
            Shape::from_dims(&[embed_dim, embed_dim]),
            0.0,
            0.1,
            device.clone(),
        )?
        ...
        let features = Tensor::randn(
            Shape::from_dims(&[batch, embed_dim]),
            0.0,
            1.0,
            device.clone(),
        )?
    ...
    // Test 3: Noise prediction loss (core diffusion objective)
    {
        ...
        let noise_pred = Tensor::randn(
            Shape::from_dims(&[batch, channels, size, size]),
            0.0,
            1.0,
            device.clone(),
        )?
    ...
    // Test 4: Multi-scale feature fusion (U-Net style)
    {
        ...
        let feat_high = Tensor::randn(
            Shape::from_dims(&[batch, base_channels, 64, 64]),
            0.0,
            1.0,
            device.clone(),
        )?
        .requires_grad_(true);
        let feat_mid = Tensor::randn(
            Shape::from_dims(&[batch, base_channels * 2, 32, 32]),
            0.0,
            1.0,
            device.clone(),
        )?
        .requires_grad_(true);
        let feat_low = Tensor::randn(
            Shape::from_dims(&[batch, base_channels * 4, 16, 16]),
            0.0,
            1.0,
            device.clone(),
        )?
        ...
        if grads.contains(feat_high.id())
            && grads.contains(feat_mid.id())
            && grads.contains(feat_low.id())
        {
