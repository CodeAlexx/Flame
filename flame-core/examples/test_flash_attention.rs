#![cfg(all(feature = "legacy_examples", feature = "flash_attn"))]
...
use flame_core::{
    flash_attention::{FlashAttention, FlashAttentionConfig},
    CudaDevice, Result, Shape, Tensor,
};
use std::sync::Arc;
...
fn main() -> Result<()> {
    ...
    // Test 2: Causal attention
    {
        ...
        println!(
            "  First position output (should be ~0.0): {:.3}",
            output_data[0]
        );
        println!(
            "  Second position output (should be ~0.5): {:.3}",
            output_data[num_heads * head_dim]
        );
    ...
    // Test 5: Edge cases
    {
        ...
        // Single head
        let q = Tensor::randn(Shape::from_dims(&[1, 16, 1, 64]), 0.0, 0.1, device.clone())?;
        let k = q.clone()?;
        let v = q.clone()?;
        ...
        let q = Tensor::randn(Shape::from_dims(&[1, 8, 4, 256]), 0.0, 0.1, device.clone())?;
        let k = q.clone()?;
        let v = q.clone()?;
