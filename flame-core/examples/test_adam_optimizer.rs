...
use flame_core::{
    adam::Adam, parameter::Parameter, AutogradContext, CudaDevice, Result, Shape, Tensor,
};
use std::sync::Arc;
    ...
    // Test 1: Simple linear regression with Adam
    {
        ...
        for epoch in 0..500 {
            ...
            for (i, (x_val, y_val)) in x_data.iter().zip(y_data.iter()).enumerate() {
                ...
                let y_true =
                    Tensor::from_vec(vec![*y_val], Shape::from_dims(&[1]), device.clone())?;
            ...
            if epoch % 100 == 0 {
                ...
                println!(
                    "  Epoch {}: loss={:.4}, w={:.3}, b={:.3}",
                    epoch, avg_loss, w_val, b_val
                );
            }
        }
        ...
        assert!(
            losses[losses.len() - 1] < losses[0] * 0.1,
            "Loss didn't decrease sufficiently"
        );
