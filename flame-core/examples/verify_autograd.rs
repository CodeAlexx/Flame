...
fn main() -> Result<()> {
    ...
    // Test 2: Multiple paths to same variable
    {
        ...
        let x = Tensor::from_vec(vec![2.0, 4.0], Shape::from_dims(&[2]), device.clone())?
            .requires_grad_(true);
    ...
    // Test 4: Broadcasting gradients
    {
        ...
        .requires_grad_(true);
        let bias = Tensor::from_vec(vec![10.0, 20.0], Shape::from_dims(&[2]), device.clone())?
            .requires_grad_(true);
    ...
    // Test 6: No gradient accumulation between backward passes
    {
        ...
        let x = Tensor::from_vec(vec![1.0, 2.0], Shape::from_dims(&[2]), device.clone())?
            .requires_grad_(true);
