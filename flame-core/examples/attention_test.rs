...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Encoder-decoder attention output shape: {:?}",
        output_enc_dec.shape().dims()
    );
    ...
    println!(
        "Time for 10 forward passes (batch=4, seq_len=128): {:?}",
        elapsed
    );
    ...
    let simple_input = Tensor::randn(Shape::from_dims(&[1, 4, 8]), 0.0, 0.1, device)?;
