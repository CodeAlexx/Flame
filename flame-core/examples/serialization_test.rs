use flame_core::{Tensor, CudaDevice, Shape, serialization::{save_tensor, load_tensor, save_tensors, load_tensors, SerializationFormat}};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create test tensors
    let tensor1 = Tensor::randn(
        Shape::from_dims(&[3, 4, 5]),
        0.0, 1.0,
        device.clone()
    )?;
    ...
    let max_diff = original_data.iter()
    ...
    let weight_diff = weight_data.iter()
    ...
    assert!(weight_diff < 1e-6 && bias_diff < 1e-6, "Loaded tensors don't match originals");
    ...
    let safe_diff = safe_original_data.iter()
        .zip(safe_data.iter())
        .map(|(a, b)| (a - b).abs())
        .max_by(|a, b| a.partial_cmp(b).unwrap())
        .unwrap();
    ...
    assert!(safe_diff < 1e-6, "SafeTensors loaded tensor doesn't match original");
    ...
    let conv_diff = conv_original_data.iter()
        .zip(conv_data.iter())
        .map(|(a, b)| (a - b).abs())
        .max_by(|a, b| a.partial_cmp(b).unwrap())
        .unwrap();
    ...
    assert!(conv_diff < 1e-6, "Convenience method loaded tensor doesn't match original");
    println!("✓ Convenience methods test passed!");
    
    // Performance test
    println!("\n=== Performance Test ===");
    let large_tensor = Tensor::randn(
        Shape::from_dims(&[512, 512, 4]),
        0.0, 1.0,
        device.clone()
    )?;
