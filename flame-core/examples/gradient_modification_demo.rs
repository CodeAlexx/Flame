...
use flame_core::{
    ...
    gradient_clip::{GradientClipper, GradientClipStrategy},
};
...
fn main() -> Result<()> {
    ...
    for (name, tensor_id) in [("w1", w1.id()), ("b1", b1.id()), ("w2", w2.id()), ("b2", b2.id())] {
    ...
    if let Some(w1_grad) = gradients.get(w1.id()) {
        ...
        let clipped: Vec<f32> = grad_vec.iter()
            .map(|&g| g.clamp(-0.5, 0.5))
            .collect();
        
        let clipped_grad = Tensor::from_vec(clipped, w1_grad.shape().clone(), device.clone())?;
        let new_norm = clipped_grad.to_vec()?.iter().map(|g| g * g).sum::<f32>().sqrt();
        
        println!("  w1 gradient norm: {:.4} -> {:.4}", original_grads["w1"], new_norm);
    ...
    if let Some(w2_grad) = gradients.get(w2.id()) {
        ...
        if grad_norm > 1.0 {
            ...
            let new_norm = normalized_grad.to_vec()?.iter().map(|g| g * g).sum::<f32>().sqrt();
            
            println!("  w2 gradient norm: {:.4} -> {:.4}", original_grads["w2"], new_norm);
    ...
    if let Some(b1_grad) = gradients.get(b1.id()) {
        ...
        let new_norm = noisy_grad.to_vec()?.iter().map(|g| g * g).sum::<f32>().sqrt();
        
        println!("  b1 gradient norm: {:.4} -> {:.4} (with noise)", orig_norm, new_norm);
    ...
    if let Some(b2_grad) = gradients.get(b2.id()) {
        ...
        let new_norm = scaled_grad.to_vec()?.iter().map(|g| g * g).sum::<f32>().sqrt();
        
        println!("  b2 gradient norm: {:.4} -> {:.4}", original_grads["b2"], new_norm);
