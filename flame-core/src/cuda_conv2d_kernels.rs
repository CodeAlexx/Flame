...
// Simple im2col for stride=1, padding=1 case with grid-stride loop
...
// Full im2col implementation with configurable stride and padding using grid-stride loop
...
// Full im2col with all parameters using grid-stride loop (split into two kernels to reduce param count)
extern "C" __global__ void im2col_kernel_v2(
    const float* data_im,
    float* data_col,
    const float* dims,  // Array containing: batch_size, channels, height, width, kernel_h, kernel_w
    const float* conv_params  // Array containing: pad_h, pad_w, stride_h, stride_w, out_h, out_w
) {
    const int batch_size = (int)dims[0];
    const int channels = (int)dims[1];
    const int height = (int)dims[2];
    const int width = (int)dims[3];
    const int kernel_h = (int)dims[4];
    const int kernel_w = (int)dims[5];
    
    const int pad_h = (int)conv_params[0];
    const int pad_w = (int)conv_params[1];
    const int stride_h = (int)conv_params[2];
    const int stride_w = (int)conv_params[3];
    const int out_h = (int)conv_params[4];
    const int out_w = (int)conv_params[5];
