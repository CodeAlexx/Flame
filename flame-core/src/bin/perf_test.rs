...
fn main() -> flame_core::Result<()> {
    ...
    println!("\n3. Testing Autograd:");
    let x =
        Tensor::randn(Shape::from_dims(&[128, 64]), 0.0, 1.0, device.clone())?.requires_grad_(true);
    let w =
        Tensor::randn(Shape::from_dims(&[64, 32]), 0.0, 0.1, device.clone())?.requires_grad_(true);

    match x.matmul(&w) {
        Ok(y) => match y.sum() {
            Ok(loss) => match loss.backward() {
