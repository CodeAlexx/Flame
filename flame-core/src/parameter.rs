...
use crate::{FlameError, Result, Shape, Tensor, TensorId};
...
impl Parameter {
    ...
    pub fn tensor(&self) -> Result<Tensor> {
        Ok(self
            .data
            .lock()
            .map_err(|_| FlameError::Training("parameter data mutex poisoned".into()))?
            .clone())
    }
    ...
    pub fn set_grad(&self, grad: Tensor) -> Result<()> {
        let mut grad_lock = self
            .grad
            .lock()
            .map_err(|_| FlameError::Training("parameter grad mutex poisoned".into()))?;
        *grad_lock = Some(grad);
        Ok(())
    }
    ...
    pub fn apply_update(&self, update: &Tensor) -> Result<()> {
        let mut data_lock = self
            ...
            .lock()
            .map_err(|_| FlameError::Training("parameter data mutex poisoned".into()))?;
        let update_cast = if update.dtype() == data_lock.dtype() {
            update.clone_result()?
        } else {
            update.to_dtype(data_lock.dtype())?
        };
        let new_data = data_lock.sub(&update_cast)?;
        *data_lock = new_data;
        Ok(())
    }
