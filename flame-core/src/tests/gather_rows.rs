...
mod tests {
    ...
    fn gather_rows_basic() {
        ...
        let tab_t = Tensor::from_vec(table.clone(), Shape::from_dims(&[v,d]), dev.cuda_device().clone()).unwrap();
        let ids = Tensor::from_vec(vec![1.0,3.0,6.0, 0.0,2.0,4.0], Shape::from_dims(&[b,s]), dev.cuda_device().clone()).unwrap()
            .to_dtype(DType::I32).unwrap();
        let out = tab_t.index_select0(&ids).unwrap().to_dtype(DType::F32).unwrap();
