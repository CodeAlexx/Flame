...
| 4 (skeleton): `RingSlabAllocator` skeleton | **DELETED 2026-05-14** | was @`98dbebc`, microbench@`31e550b` — off-spec wiring (BlockOffloader weight slots, not the activation path the spec called for), verified ineffective for the Klein step-2 crash. |
| 4 (restart): `flame_core::ring_alloc` faithful OT port | **PHASE 1 SHIPPED 2026-05-14** | flame-core@`2927038` (core: design doc + impl + 9 tests), `828c98f` (Bug Fixer: silent wrap-and-lap fix + 6 tests = 15 total). Skeptic in-flight. Phase 2 (consumer migration + Klein 30-step gate, replace `FLAME_ALLOC_POOL=0` workaround) pending. Faithful port of OT `StaticLayerAllocator`/`StaticLayerTensorAllocator` (LayerOffloadConductor.py:37-222) — what the deleted skeleton should have been. |
...
**Live design**: Phase 1 + 2 (activation-side primitives) shipped and exercised end-to-end in Klein 9B. **Phase 4 restart**: `flame_core::ring_alloc` is a clean faithful OT port (not the deleted skeleton). Phase 1 of the restart shipped (`2927038`+`828c98f`); Phase 2 wires it as a `cuda_alloc_pool` replacement for offload-pattern allocations, target = Klein 9B 30-step run without `FLAME_ALLOC_POOL=0`. The deleted skeletons (Phase 3 `OffloadCoordinator`, Phase 5 `BlockOffloadStrategy`) are NOT being restarted — `BlockOffloader`'s existing strategies cover the layer-side offload, and the coordinator wrapping added complexity without value.
