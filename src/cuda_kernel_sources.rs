...
pub const PERMUTE_NHWC_TO_NCHW_KERNEL_BF16: &str = r#"
#include <cuda_bf16.h>

extern "C" __global__ void permute_nhwc_to_nchw_kernel_bf16(
...
pub const FLIP_LAST_DIM_F32_KERNEL: &str = r#"
extern "C" __global__ void flip_last_dim_f32_kernel(
    const float* input,
    float* output,
    int rows,
    int cols
    ...
    if (idx < total) {
        int row = idx / cols;
        int col = idx % cols;
        int src = row * cols + (cols - 1 - col);
        output[idx] = input[src];
    }
}
"#;

pub const FLIP_LAST_DIM_BF16_KERNEL: &str = r#"
#include <cuda_bf16.h>

extern "C" __global__ void flip_last_dim_bf16_kernel(
    const __nv_bfloat16* input,
    __nv_bfloat16* output,
    int rows,
    int cols
...
pub const FLOOR_KERNEL: &str = r#"
extern "C" __global__ void floor_kernel(
    const float* input,
    float* output,
    int n
) {
...
pub const CEIL_KERNEL: &str = r#"
extern "C" __global__ void ceil_kernel(
    const float* input,
    float* output,
    int n
) {
...
pub const ROUND_KERNEL: &str = r#"
extern "C" __global__ void round_kernel(
    const float* input,
    float* output,
    int n
) {
...
pub const SLICE_KERNEL_BF16: &str = r#"
extern "C" __global__ void slice_kernel_bf16(
...
#[cfg(feature = "bf16_u16")]
pub const GATE_ADD_BF16_INPLACE_KERNEL: &str = r#"
#include <cuda_bf16.h>

extern "C" __global__ void gate_add_bf16_inplace_kernel(
    __nv_bfloat16* dst,
    ...
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int total = batch * tokens * hidden;
    if (idx < total) {
        int h = idx % hidden;
        int b = idx / (tokens * hidden);
        // gate is [batch, hidden]
        int gate_idx = b * hidden + h;
        dst[idx] = __hadd(dst[idx], gate[gate_idx]);
    }
}
"#;

pub const PERMUTE_GENERIC_F32_KERNEL: &str = r#"
extern "C" __global__ void permute_generic_f32_kernel(
    ...
    while (idx < total) {
        ...
        for (int d = 0; d < ndim; ++d) {
            ...
            int src_axis = perm[d];
            src_index += coord * in_strides[src_axis];
        }
...
#[cfg(feature = "bf16_u16")]
pub const PERMUTE_GENERIC_BF16_KERNEL: &str = r#"
extern "C" __global__ void permute_generic_bf16_kernel(
...
pub const PERMUTE_NCHW_TO_NHWC_KERNEL_BF16: &str = r#"
#include <cuda_bf16.h>

extern "C" __global__ void permute_nchw_to_nhwc_kernel_bf16(
    const __nv_bfloat16* input,
    __nv_bfloat16* output,
    int batch,
    int channels,
