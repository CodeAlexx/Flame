...
pub fn flash_attention_forward(
    ...
    if query.dtype() != DType::BF16 || key.dtype() != DType::BF16 || value.dtype() != DType::BF16 {
        return Err(Error::InvalidInput(
            "flash_attention_forward expects BF16 tensors at the public boundary".into(),
        ));
    }
...
fn flash_attention_forward_kernel_small(
    ...
    let f = query
        .device
        .get_func("flash_attn_fwd_small", "flash_attn_fwd_small")
        .ok_or_else(|| Error::Cuda("missing kernel: flash_attn_fwd_small".into()))?;
    ...
    let mask_ptr = if let Some(m) = attention_mask {
...
fn flash_attention_forward_kernel_medium(
    ...
    let f = query
        .device
        .get_func("flash_attn_fwd_medium", "flash_attn_fwd_medium")
        .ok_or_else(|| Error::Cuda("missing kernel: flash_attn_fwd_medium".into()))?;
...
pub fn chunked_attention_forward(
    ...
    let f = query
        .device
        .get_func("chunked_attn_fwd", "chunked_attn_fwd")
        .ok_or_else(|| Error::Cuda("missing kernel: chunked_attn_fwd".into()))?;

    let shape = query.shape().dims();
    let (batch_size, num_heads, seq_len_q, head_dim) = (shape[0], shape[1], shape[2], shape[3]);
