...
/// Derive the storage dtype for optimizer state given the parameter dtype and current config.
pub fn select_optimizer_state_dtype(param_dtype: DType) -> DType {
    #[cfg(feature = "true_bf16_optimizer_states")]
    {
        let requested = optimizer_moment_dtype();
        if param_dtype == DType::BF16 && requested == DType::BF16 {
            return DType::BF16;
        }
    }
