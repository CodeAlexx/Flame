...
use crate::{Tensor, Shape, Result, Error, DType};
use crate::cudnn::{
...
pub fn cudnn_matmul(a: &Tensor, b: &Tensor) -> Result<Tensor> {
    ...
    let handle_guard = handle.lock().map_err(|_| Error::Training("cudnn handle mutex poisoned".into()))?;
    ...
    if status != crate::cudnn::status::CUDNN_STATUS_SUCCESS {
        return Err(Error::CudaError(format!("cudnn_matmul failed with status: {}", status)));
    }
...
fn create_matmul_op_descriptor() -> Result<cudnnOpTensorDescriptor_t> {
    ...
    if status != crate::cudnn::status::CUDNN_STATUS_SUCCESS {
        return Err(Error::CudaError(format!("Failed to create op tensor descriptor: {}", status)));
    }
    ...
    if status != crate::cudnn::status::CUDNN_STATUS_SUCCESS {
        ...
        return Err(Error::CudaError(format!("Failed to set op tensor descriptor: {}", status)));
    }
