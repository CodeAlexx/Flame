...
use crate::cudnn::{
    ...
    descriptors::{ConvolutionDescriptor, FilterDescriptor, TensorDescriptor},
...
pub fn cudnn_conv2d(
    ...
    if status != 0 {
        ...
        eprintln!(
            "cuDNN {} failed (status {}), trying fallback",
        ...
        if status != 0 {
            return Err(Error::CudaError(format!(
                "Failed to get workspace size for cuDNN convolution: {}",
                status
            )));
        }
    }
    ...
    if status != 0 {
        ...
        if algo != crate::cudnn::algorithms::CUDNN_CONVOLUTION_FWD_ALGO_IMPLICIT_GEMM {
            eprintln!(
                "cuDNN {} failed (status {}), trying IMPLICIT_GEMM",
                AlgorithmSelector::algorithm_name(algo),
                status
            );
            ...
            if status != 0 {
                return Err(Error::CudaError(format!(
                    "cuDNN convolution failed with all algorithms: {}",
                    status
                )));
            }
        } else {
    ...
    if let Some(bias) = bias {
        ...
        if status != 0 {
            ...
            eprintln!(
                "Warning: cuDNN bias addition failed (status {}), continuing without bias",
                status
            );
        }
    }
