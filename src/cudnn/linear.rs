...
pub fn is_cudnn_linear_compatible(input: &Tensor, weight: &Tensor, _bias: Option<&Tensor>) -> bool {
    // Check if cuDNN is available
    if !is_cudnn_available() {
        return false;
    }

    // Current cuDNN linear shim is FP32-only; defer to BF16 kernel otherwise.
...
pub fn cudnn_linear(input: &Tensor, weight: &Tensor, bias: Option<&Tensor>) -> Result<Tensor> {
    ...
    let _output_shape = match input_shape.dims().len() {
        2 => {
        ...
        _ => {
            return Err(Error::InvalidOperation(format!(
                "Invalid input dimensions for linear: {:?}",
                input_shape
            )))
        }
    };
...
pub fn cudnn_batched_linear(
    ...
    inputs
        ...
        .zip(weights.iter())
        .zip(biases.iter())
        .map(|((input, weight), bias)| cudnn_linear(input, weight, *bias))
        .collect()
}
