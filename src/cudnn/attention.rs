...
pub fn is_cudnn_attention_compatible(
    ...
    q_shape.dims().len() >= 3 && k_shape.dims().len() >= 3 && v_shape.dims().len() >= 3
}
...
pub fn cudnn_scaled_dot_product_attention(
    ...
    // Get dimensions
    let (_batch_size, _num_heads, seq_len, head_dim) = {
...
pub fn cudnn_multi_head_attention(
    ...
    let attention_output =
        cudnn_scaled_dot_product_attention(&q, &k, &v, attention_mask, dropout_p, is_causal, None)?;
