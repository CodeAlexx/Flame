...
// Legacy autograd ops; keep compiled but unused until rewritten.
...
use crate::tensor::contracts::{assert_nhwc_bf16_public, scalar_for_dtype};
...
pub fn layer_norm_backward(
    ...
    guard_tensor("layer_norm_backward grad_output", grad_output)?;
    guard_tensor("layer_norm_backward input", input)?;
    guard_tensor("layer_norm_backward normalized", normalized)?;
    guard_optional_tensor("layer_norm_backward weight", weight)?;
    guard_optional_tensor("layer_norm_backward bias", bias)?;
    guard_tensor("layer_norm_backward mean", mean)?;
    guard_tensor("layer_norm_backward var", var)?;
    if input.dtype() == DType::BF16 && grad_output.dtype() == DType::BF16 {
        let (dx, dgamma, dbeta) = cuda_ops_bf16::layer_norm_backward_bf16(
            input,
            grad_output,
            weight,
            bias,
    ...
    mean_grad_normalized = mean_grad_normalized.div_scalar(scalar_for_dtype(
        norm_size as f32,
        mean_grad_normalized.dtype(),
    ))?;
    ...
    dot_p = dot_p.div_scalar(scalar_for_dtype(norm_size as f32, dot_p.dtype()))?;
    ...
    let grad_bias = if bias.is_some() {
        Some(grad_output.sum_dims(&(0..axis).collect::<Vec<_>>())?)
    } else {
        None
    };

    let grad_input = ensure_bf16(grad_input)?;
    let grad_weight = match grad_weight {
        Some(gw) => Some(ensure_bf16(gw)?),
        None => None,
    };
    let grad_bias = match grad_bias {
        Some(gb) => Some(ensure_bf16(gb)?),
        None => None,
    };
...
pub fn batch_norm_backward(
    ...
    guard_tensor("batch_norm_backward grad_output", grad_output)?;
    guard_tensor("batch_norm_backward input", input)?;
    guard_optional_tensor("batch_norm_backward weight", weight)?;
    guard_tensor("batch_norm_backward running_mean", running_mean)?;
    guard_tensor("batch_norm_backward running_var", running_var)?;
    let _device = input.device();
    let shape = input.shape().dims();
    ...
    if !training {
        ...
        let grad_weight = if weight.is_some() {
            ...
            Some(
                grad_output.mul(&normalized)?.sum_dims(&[0, 2, 3])?, // Sum over batch and spatial dims
            )
        } else {
            None
        };
    ...
    let total_size_scalar =
        scalar_for_dtype((batch_size * spatial_size) as f32, grad_output.dtype());
    ...
    let mean_dy_xmu = grad_out_scaled
        .mul(&normalized)?
    ...
    let grad_input = grad_out_scaled
        ...
        .mul(&inv_std)?
        .div_scalar(total_size_scalar)?;

    // Weight and bias gradients
    let grad_weight = if weight.is_some() {
        Some(grad_output.mul(&normalized)?.sum_dims(&[0, 2, 3])?)
    } else {
        None
    };
...
pub fn softmax_backward(output: &Tensor, grad_output: &Tensor, dim: isize) -> Result<Tensor> {
    guard_tensor("softmax_backward output", output)?;
    guard_tensor("softmax_backward grad_output", grad_output)?;
    // For softmax: grad_input = output * (grad_output - sum(grad_output * output))
    let sum_term = grad_output.mul(output)?.sum_keepdim(dim)?;

    let grad_input = output.mul(&grad_output.sub(&sum_term)?)?;
    ensure_bf16(grad_input)
}

/// Compute gradients for LogSoftmax operation
pub fn log_softmax_backward(grad_output: &Tensor, output: &Tensor, dim: isize) -> Result<Tensor> {
    guard_tensor("log_softmax_backward grad_output", grad_output)?;
    guard_tensor("log_softmax_backward output", output)?;
...
pub fn dropout_backward(
    ...
    guard_tensor("dropout_backward grad_output", grad_output)?;
    if !training || dropout_prob == 0.0 {
        return ensure_bf16(grad_output.clone_result()?);
    }

    // Scale by keep probability and apply mask
    let scale = 1.0 / (1.0 - dropout_prob);
    let grad = grad_output.mul(mask)?.mul_scalar(scale)?;
    ensure_bf16(grad)
}
...
pub fn gelu_backward(grad_output: &Tensor, input: &Tensor) -> Result<Tensor> {
    guard_tensor(
        "autograd_ops_complete::gelu_backward grad_output",
        grad_output,
    )?;
    guard_tensor("autograd_ops_complete::gelu_backward input", input)?;
    const INV_SQRT_2PI: f32 = 0.398_942_3_f32;
    let sqrt_2 = std::f32::consts::SQRT_2;

    let x = input;
    let erf_term = x.mul_scalar(1.0 / sqrt_2)?.erf()?;
    let cdf = erf_term.add_scalar(1.0)?.mul_scalar(0.5)?;
    let pdf = x
        .square()?
        .mul_scalar(-0.5)?
        .exp()?
        .mul_scalar(INV_SQRT_2PI)?;
    let deriv = cdf.add(&x.mul(&pdf)?)?;
    let grad = grad_output.mul(&deriv)?;
    ensure_bf16(grad)
}
...
pub fn silu_backward(grad_output: &Tensor, input: &Tensor) -> Result<Tensor> {
    guard_tensor(
        "autograd_ops_complete::silu_backward grad_output",
        grad_output,
    )?;
    guard_tensor("autograd_ops_complete::silu_backward input", input)?;
    // SiLU(x) = x * sigmoid(x)
    // SiLU'(x) = sigmoid(x) + x * sigmoid(x) * (1 - sigmoid(x))

    let sigmoid = input.sigmoid()?;
    let one_minus_sigmoid = sigmoid.neg()?.add_scalar(1.0)?;
    let derivative = sigmoid
        .clone_result()?
        .add(&input.mul(&sigmoid)?.mul(&one_minus_sigmoid)?)?;
    let grad = grad_output.mul(&derivative)?;
    ensure_bf16(grad)
}

/// Compute gradients for Tanh operation
pub fn tanh_backward(grad_output: &Tensor, output: &Tensor) -> Result<Tensor> {
    guard_tensor(
        "autograd_ops_complete::tanh_backward grad_output",
        grad_output,
    )?;
    guard_tensor("autograd_ops_complete::tanh_backward output", output)?;
    // tanh'(x) = 1 - tanh(x)^2
    let one_minus_tanh_sq = output.square()?.neg()?.add_scalar(1.0)?.clamp(1e-7, 1.0)?; // Prevent vanishing gradients
    let grad = grad_output.mul(&one_minus_tanh_sq)?;
    ensure_bf16(grad)
}

/// Compute gradients for Sigmoid operation with stability
pub fn sigmoid_backward(grad_output: &Tensor, output: &Tensor) -> Result<Tensor> {
    guard_tensor(
        "autograd_ops_complete::sigmoid_backward grad_output",
        grad_output,
    )?;
    guard_tensor("autograd_ops_complete::sigmoid_backward output", output)?;
    // sigmoid'(x) = sigmoid(x) * (1 - sigmoid(x))
    let one_minus_sigmoid = output.neg()?.add_scalar(1.0)?;
    let derivative = output.mul(&one_minus_sigmoid)?.clamp(1e-7, 0.25)?; // Max derivative of sigmoid is 0.25
    let grad = grad_output.mul(&derivative)?;
    ensure_bf16(grad)
}

/// Compute gradients for general broadcasting operation
pub fn broadcast_backward(grad_output: &Tensor, original_shape: &[usize]) -> Result<Tensor> {
    guard_tensor("broadcast_backward grad_output", grad_output)?;
...
pub fn reduce_grad_to_shape(grad: &Tensor, target_shape: &[usize]) -> Result<Tensor> {
    guard_tensor("reduce_grad_to_shape grad", grad)?;
    let grad_shape = grad.shape().dims();

    if grad_shape == target_shape {
        return ensure_bf16(grad.clone_result()?);
    }
...
impl Tensor {
    ...
    pub fn var_dims(&self, dims: &[usize], keepdim: bool, unbiased: bool) -> Result<Tensor> {
        ...
        let divisor_scalar = if unbiased && n > 1 {
            scalar_for_dtype((n - 1) as f32, sum_sq.dtype())
        } else {
            scalar_for_dtype(n as f32, sum_sq.dtype())
        };

        let var = sum_sq.div_scalar(divisor_scalar)?;
    ...
    pub fn mean_dims(&self, dims: &[usize], keepdim: bool) -> Result<Tensor> {
        ...
        let n: usize = dims.iter().map(|&d| self.shape().dims()[d]).product();
        let mean = sum.div_scalar(scalar_for_dtype(n as f32, sum.dtype()))?;
    ...
    pub fn clamp_with_grad(&self, min: f32, max: f32) -> Result<Tensor> {
        ...
        let clamped: Vec<f32> = data.iter().map(|&x| x.clamp(min, max)).collect();
...
pub fn clamp_backward(grad_output: &Tensor, input: &Tensor, min: f32, max: f32) -> Result<Tensor> {
    guard_tensor("clamp_backward grad_output", grad_output)?;
    guard_tensor("clamp_backward input", input)?;
    ...
    let mask_tensor = ensure_bf16(mask_tensor)?;
    let grad = grad_output.mul(&mask_tensor)?;
    ensure_bf16(grad)
}
...
pub fn group_norm_backward(
    ...
    guard_tensor("group_norm_backward var", var)?;
    guard_optional_tensor("group_norm_backward weight", weight)?;
    let shape = input.shape().dims();
    let batch_size = shape[0];
    ...
    let f = input
        .device
        .get_func("group_norm_backward", "group_norm_backward")
        .ok_or_else(|| Error::Cuda("Failed to get group_norm_backward kernel".into()))?;

    // Allocate gradient tensors
    let grad_input_data =
        crate::tensor::alloc_zeros_from_pool(&input.device, input.shape().elem_count())?;
    let grad_weight_data = if weight.is_some() {
        Some(crate::tensor::alloc_zeros_from_pool(
            &input.device,
            num_channels,
        )?)
    } else {
    ...
    let weight_ptr = if let Some(w) = weight {
        w.storage.try_as_slice_f32()?
    } else {
        &dummy
    };

    let grad_weight_ptr = grad_weight_data.as_ref().map(|d| d as &_).unwrap_or(&dummy);

    // Pack dimensions to reduce parameter count
    let dims1 = ((batch_size as i32) << 16) | (num_channels as i32);
    let dims2 = ((num_groups as i32) << 16) | (channels_per_group as i32);
    let dims3 = ((spatial_size as i32) << 1) | i32::from(weight.is_some());
...
pub fn flash_attention_backward(
    ...
    guard_tensor("flash_attention_backward grad_output", grad_output)?;
    guard_tensor("flash_attention_backward query", query)?;
    guard_tensor("flash_attention_backward key", key)?;
    guard_tensor("flash_attention_backward value", value)?;
    ...
    let grad_q = ensure_bf16(grad_q)?;
    let grad_k = ensure_bf16(grad_k)?;
    let grad_v = ensure_bf16(grad_v)?;
