...
impl AutogradEngine {
    ...
    pub fn register_tensor(&mut self, tensor: Arc<Mutex<Tensor>>, op: Option<Op>) -> TensorId {
        GRAPH.with(|graph| {
            let mut graph = graph.lock().map_err(|_| crate::Error::Training("autograd graph mutex poisoned".into()))?;
            let tensor_ref = tensor.lock().map_err(|_| crate::Error::Training("autograd tensor mutex poisoned".into()))?;
    ...
    pub fn backward(&self, loss_tensor: &Tensor) -> Result<()> {
        ...
        if loss_tensor.rank() == 4 {
            assert_nhwc_bf16_public("autograd_engine::backward loss", loss_tensor)?;
        }
        ...
        let gradients = GRAPH.with(|graph| {
            let graph = graph.lock().map_err(|_| crate::Error::Training("autograd graph mutex poisoned".into()))?;
        ...
        for (tensor_id, grad) in gradients {
            if let Some(tensor_arc) = tensor_map.get(&tensor_id) {
                let mut tensor = tensor_arc.lock().map_err(|_| crate::Error::Training("autograd tensor mutex poisoned".into()))?;
...
pub mod ops {
    ...
    pub fn add_tracked(lhs: &Tensor, rhs: &Tensor) -> Result<Tensor> {
        ...
        if lhs.requires_grad || rhs.requires_grad {
            ENGINE.with(|engine| {
                let mut engine = engine.lock().map_err(|_| crate::Error::Training("autograd engine mutex poisoned".into()))?;
                ...
                if result_mut.rank() == 4 {
                    assert_nhwc_bf16_public("autograd_engine::ops::add_tracked out", &result_mut)?;
                }
                Ok(result_mut)
            })
        } else {
            if result.rank() == 4 {
                assert_nhwc_bf16_public("autograd_engine::ops::add_tracked out", &result)?;
            }
            Ok(result)
        }
    }
    ...
    pub fn mul_tracked(lhs: &Tensor, rhs: &Tensor) -> Result<Tensor> {
        ...
        if lhs.requires_grad || rhs.requires_grad {
            ENGINE.with(|engine| {
                ...
                result_mut.requires_grad = true;
                if result_mut.rank() == 4 {
                    assert_nhwc_bf16_public("autograd_engine::ops::mul_tracked out", &result_mut)?;
                }
                Ok(result_mut)
            })
        } else {
            if result.rank() == 4 {
                assert_nhwc_bf16_public("autograd_engine::ops::mul_tracked out", &result)?;
            }
            Ok(result)
        }
    }
    ...
    pub fn matmul_tracked(lhs: &Tensor, rhs: &Tensor) -> Result<Tensor> {
        ...
        if lhs.requires_grad || rhs.requires_grad {
            ENGINE.with(|engine| {
                ...
                result_mut.requires_grad = true;
                if result_mut.rank() == 4 {
                    assert_nhwc_bf16_public("autograd_engine::ops::matmul_tracked out", &result_mut)?;
                }
                Ok(result_mut)
            })
        } else {
            if result.rank() == 4 {
                assert_nhwc_bf16_public("autograd_engine::ops::matmul_tracked out", &result)?;
            }
            Ok(result)
        }
    }
    ...
    pub fn mean_tracked(input: &Tensor) -> Result<Tensor> {
        ...
        if input.requires_grad {
            ...
            ENGINE.with(|engine| {
                ...
                result_mut.requires_grad = true;
                if result_mut.rank() == 4 {
                    assert_nhwc_bf16_public("autograd_engine::ops::mean_tracked out", &result_mut)?;
                }
                Ok(result_mut)
            })
        } else {
            if result.rank() == 4 {
                assert_nhwc_bf16_public("autograd_engine::ops::mean_tracked out", &result)?;
            }
            Ok(result)
        }
    }
    ...
    pub fn create_tracked_tensor(data: Vec<f32>, shape: Shape, device: Arc<cudarc::driver::CudaDevice>, requires_grad: bool) -> Result<Tensor> {
        ...
        if tensor.rank() == 4 {
            assert_nhwc_bf16_public("autograd_engine::ops::create_tracked_tensor out", &tensor)?;
        }
...
mod tests {
    ...
    fn test_autograd_simple() -> Result<()> {
        ...
        let gradients = ENGINE.with(|engine| {
            let engine = engine.lock().map_err(|_| crate::Error::Training("autograd engine mutex poisoned".into()))?;
