use crate::{Shape, DType, Result, Error};
...
impl CudaTensor {
    ...
    pub fn add(&self, other: &CudaTensor) -> Result<CudaTensor> {
        ...
        if self.shape == other.shape {
            ...
            return Err(Error::InvalidOperation("Broadcasting not implemented yet".into()));
        }
    ...
    pub fn item(&self) -> Result<f32> {
        if self.shape.elem_count() != 1 {
            return Err(Error::InvalidOperation("item() requires tensor with single element".into()));
        }
        Ok(self.to_vec()?[0])
    }
}
