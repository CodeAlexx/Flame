...
impl AutogradContext {
    pub fn record_op(
        ...
        let mut ctx = AUTOGRAD_CONTEXT.lock().map_err(|_| crate::Error::Training("autograd context mutex poisoned".into()))?;
    ...
    pub fn backward(loss: &Tensor) -> Result<GradientMap> {
        ...
        if loss.rank() == 4 {
            assert_nhwc_bf16_public("autograd_debug::backward loss", loss)?;
        }
            ...
            for (i, entry) in ctx.tape.iter().rev().enumerate() {
                ...
                if let Some(output_grad) = gradients.get(entry.output_id) {
                    ...
                    let input_grads = match &entry.op {
                        Op::Add { lhs, rhs, .. } => {
                            assert_nhwc_bf16_public("autograd_debug::add out", &output_grad)?;
                            vec![
                        ...
                        Op::Mul { lhs, rhs } => {
                            ...
                            assert_nhwc_bf16_public("autograd_debug::mul saved lhs", lhs_tensor)?;
                            assert_nhwc_bf16_public("autograd_debug::mul saved rhs", rhs_tensor)?;
                            assert_nhwc_bf16_public("autograd_debug::mul out", &output_grad)?;
                            vec![
                    ...
                    for (tensor_id, mut grad) in input_grads {
