...
impl AMPContext {
    ...
    pub fn update_scale(&mut self, found_inf: bool) {
        ...
        if found_inf {
            ...
            println!(
                "Gradient overflow detected. Decreasing loss scale to {}",
                self.loss_scale
            );
        } else {
...
impl MixedPrecisionTensor {
    ...
    pub fn update_master(&mut self, update: &Tensor) -> Result<()> {
        ...
        if self.amp_context.enabled {
            self.compute = Some(HalfTensor::from_f32(
                &self.master,
                self.amp_context.compute_dtype,
            )?);
        }
    ...
    pub fn from_f32(tensor: &Tensor, dtype: DType) -> Result<Self> {
        ...
        match dtype {
            DType::F16 => {
                ...
                let half_data: Vec<f16> = data.iter().map(|&x| f16::from_f32(x)).collect();
            ...
            DType::BF16 => {
                ...
                let half_data: Vec<bf16> = data.iter().map(|&x| bf16::from_f32(x)).collect();
            ...
            _ => Err(Error::InvalidOperation(format!(
                "Cannot create HalfTensor with dtype {:?}",
                dtype
            ))),
        }
    }
