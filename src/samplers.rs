...
// Legacy diffusion samplers kept for reference; cleanup scheduled in Phase 3.
...
impl DiffusionSampler for EulerSampler {
    fn step(
        ...
        let step_index = self
            ...
            .ok_or_else(|| Error::InvalidOperation("Timestep not found".into()))?;
...
impl DPMPlusPlus2MSampler {
    ...
    fn get_pred_x0(&self, model_output: &Tensor, timestep: f32, sample: &Tensor) -> Result<Tensor> {
        ...
        match self.prediction_type {
            ...
            PredictionType::Sample => Ok(model_output.clone_result()?),
        }
    }
}
...
impl DiffusionSampler for DPMPlusPlus2MSampler {
    fn step(
        ...
        if step_index == 0 {
            ...
            let sample_next = pred_x0
                .mul_scalar(sigma_ratio)?
            ...
            let sample_next = sample
                .mul_scalar(sigma_next / sigma)?
...
impl DDIMSampler {
    pub fn new(num_train_timesteps: usize, prediction_type: PredictionType, eta: f32) -> Self {
        Self {
            num_train_timesteps,
            num_inference_steps: 50,
            timesteps: Vec::new(),
...
impl DiffusionSampler for DDIMSampler {
    fn step(
        ...
        let pred_sample_direction =
            model_output.mul_scalar((beta_prod_t_prev - std_dev * std_dev).sqrt())?;

        // Compute previous sample
        let prev_sample = pred_x0
        ...
        let betas: Vec<f32> = match noise_schedule.as_str() {
            "linear" => {
                ...
                (0..num_train_timesteps)
                    .map(|i| {
                        beta_start
                            + (beta_end - beta_start) * i as f32 / (num_train_timesteps - 1) as f32
                    })
                    .collect()
            }
            "cosine" => {
                ...
                (0..num_train_timesteps)
                    .map(|i| {
                        ...
                        let alpha_bar = ((t + s) / (1.0 + s) * std::f32::consts::PI * 0.5)
                            .cos()
                            .powi(2);
                        let alpha_bar_prev = if i > 0 {
                            let t_prev = (i - 1) as f32 / (num_train_timesteps - 1) as f32;
                            ((t_prev + s) / (1.0 + s) * std::f32::consts::PI * 0.5)
                                .cos()
                                .powi(2)
                        } else {
                            1.0
...
pub fn create_sampler(
    ...
    match sampler_type.to_lowercase().as_str() {
        "euler" => Ok(Box::new(EulerSampler::new(
            num_train_timesteps,
            prediction_type,
        ))),
        "dpm++2m" | "dpmpp2m" => Ok(Box::new(DPMPlusPlus2MSampler::new(
            num_train_timesteps,
            prediction_type,
        ))),
        "ddim" => Ok(Box::new(DDIMSampler::new(
            num_train_timesteps,
            prediction_type,
            0.0,
        ))),
