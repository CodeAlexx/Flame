...
fn main() {
    ...
    let y3 = conv_transpose1d(x3, ref_map.get("t3_w").unwrap(), None, 2, 0, 0, c3).unwrap();
    check(
        "Test 3 k=12 s=2 groups=C",
        &y3,
        ref_map.get("t3_y").unwrap(),
        0.02,
    );
    ...
    .unwrap();
    check(
        "Test 4 k=4 s=2 p=1 op=1",
        &y4,
        ref_map.get("t4_y").unwrap(),
        0.02,
    );
