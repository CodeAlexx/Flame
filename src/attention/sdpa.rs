...
pub struct AttentionBuffers<'a> {
    pub q: Option<&'a mut Tensor>,
    pub k: Option<&'a mut Tensor>,
    pub v: Option<&'a mut Tensor>,
    pub out: Option<&'a mut Tensor>,
}
...
impl MultiHeadAttention {
    ...
    pub fn forward_with_buffers(
        ...
        value: &Tensor,
        mask: Option<&Tensor>,
        mut buffers: AttentionBuffers<'_>,
    ) -> Result<Tensor> {
        if query.dtype() != DType::BF16
            || key.dtype() != DType::BF16
            || value.dtype() != DType::BF16
        {
            return Err(Error::InvalidInput(
                "MultiHeadAttention::forward_with_buffers expects BF16 tensors at the public boundary".into(),
            ));
        }
        ...
        let q = if let Some(buf_ref) = buffers.q.as_mut() {
            let buf = &mut **buf_ref;
            self.q_proj.forward_into(query, buf)?;
            buf.clone_result()?
        } else {
            self.q_proj.forward(query)?
        };
        let k = if let Some(buf_ref) = buffers.k.as_mut() {
            let buf = &mut **buf_ref;
            self.k_proj.forward_into(key, buf)?;
            buf.clone_result()?
        } else {
            self.k_proj.forward(key)?
        };
        let v = if let Some(buf_ref) = buffers.v.as_mut() {
            let buf = &mut **buf_ref;
            self.v_proj.forward_into(value, buf)?;
            buf.clone_result()?
        } else {
            self.v_proj.forward(value)?
        };
        ...
        let mut output = if let Some(out_ref) = buffers.out.as_mut() {
            let buf = &mut **out_ref;
            self.out_proj.forward_into(&scores, buf)?;
            trap_is_bf16("MultiHeadAttention::forward_with_buffers out", buf)?;
            buf.clone_result()?
        } else {
            let mut output = self.out_proj.forward(&scores)?;
            if output.dtype() != DType::BF16 {
...
pub fn attention_impl(
    ...
    scope("sdpa.attention_impl", GuardMode::env_default(), || {
        ...
        let mut scaled: Option<Tensor> = None;
        let q_ref = if (scale_factor - 1.0).abs() < f32::EPSILON {
            q
        } else {
            scaled = Some(q.mul_scalar(scale_factor)?);
            scaled.as_ref().unwrap()
        };

        #[cfg(feature = "autograd_v4")]
        match crate::sdpa::forward_v4(q_ref, k, v, mask, causal, Some(target_scale), None) {
...
pub fn sdpa(q: &Tensor, k: &Tensor, v: &Tensor, mask: Option<&Tensor>) -> Result<Tensor> {
    trap_is_bf16("sdpa::q", q)?;
    trap_is_bf16("sdpa::k", k)?;
...
impl GeGLU {
    ...
    pub fn forward(&self, x: &Tensor) -> Result<Tensor> {
        ...
        let value = projected.narrow(dims.len() - 1, 0, half_dim)?;
...
impl FeedForward {
    pub fn new(
        ...
        let project_in = Linear::new(dim, inner_dim, true, &device)?;
    ...
    #[cfg(all(feature = "cuda", feature = "bf16_u16"))]
    pub fn forward_into(&self, x: &Tensor, hidden: &mut Tensor, output: &mut Tensor) -> Result<()> {
        scope(
            "feed_forward.forward_into",
            GuardMode::env_default(),
            || {
                trap_is_bf16("FeedForward::forward_into in", x)?;
                trap_is_bf16("FeedForward::forward_into hidden", hidden)?;
                trap_is_bf16("FeedForward::forward_into out", output)?;

                if !Arc::ptr_eq(x.device(), hidden.device())
                    || !Arc::ptr_eq(x.device(), output.device())
                {
                    return Err(Error::InvalidInput(
                        "FeedForward::forward_into expects tensors on the same device".into(),
                    ));
                }

                if x.requires_grad() || hidden.requires_grad() || output.requires_grad() {
                    return Err(Error::InvalidOperation(
                        "FeedForward::forward_into does not support autograd-enabled tensors"
                            .into(),
                    ));
                }
                ...
                if input_dims.is_empty() {
                    return Err(Error::InvalidInput(
                        "FeedForward::forward_into expects rank >= 1 tensors".into(),
                    ));
                }

                let mut expected_hidden = input_dims.to_vec();
                *expected_hidden.last_mut().unwrap() = self.project_in.out_features();
                if hidden.shape().dims() != expected_hidden {
                    return Err(Error::ShapeMismatch {
                        expected: Shape::from_dims(&expected_hidden),
                        got: hidden.shape().clone(),
                    });
                }

                let mut expected_out = input_dims.to_vec();
                *expected_out.last_mut().unwrap() = self.linear.out_features();
                if output.shape().dims() != expected_out {
                    return Err(Error::ShapeMismatch {
                        expected: Shape::from_dims(&expected_out),
                        got: output.shape().clone(),
                    });
                }

                self.project_in.forward_into(x, hidden)?;

                let hidden_view = hidden.clone_result()?;
                crate::cuda_ops_bf16::gelu_bf16_into(&hidden_view, hidden)?;

                self.linear.forward_into(hidden, output)?;
                trap_is_bf16("FeedForward::forward_into out", output)?;
                Ok(())
            },
        )
    ...
    pub fn forward_into(&self, x: &Tensor, hidden: &mut Tensor, output: &mut Tensor) -> Result<()> {
        Err(Error::Unsupported(
            "FeedForward::forward_into requires cuda + bf16_u16 features".into(),
        ))
    }
}
