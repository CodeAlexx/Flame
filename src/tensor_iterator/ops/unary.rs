...
// =============================================================================
// gelu_exact: exact-erf GELU `y = 0.5 * x * (1 + erff(x/√2))`.
// Matches `torch.nn.GELU()` default (`approximate='none'`). Used by
// Cosmos-Predict2.5 for PyTorch parity. Forward-only — no backward
// registered (Cosmos is inference-only).
// =============================================================================

crate::declare_stub!(pub GELU_EXACT_STUB);

#[cfg(feature = "cuda")]
pub(crate) fn gelu_exact_bf16_kernel(iter: &mut TensorIteratorBase<'_>) -> Result<()> {
    let meta = iter.build_iter_metadata()?;
    let stream = iter.stream()?;
    let status = unsafe { flame_gelu_exact_bf16_kernel(&meta, stream) };
    if status != 0 {
        return Err(Error::Cuda(format!(
            "flame_gelu_exact_bf16_kernel failed with code {}",
            status
        )));
    }
    Ok(())
...
#[cfg(not(feature = "cuda"))]
pub(crate) fn gelu_exact_bf16_kernel(_iter: &mut TensorIteratorBase<'_>) -> Result<()> {
    Err(Error::InvalidOperation(
        "gelu_exact_bf16_kernel requires the cuda feature".into(),
    ))
}

pub fn gelu_exact_bf16_iter(x: &Tensor) -> Result<Tensor> {
    debug_assert_eq!(
        x.dtype(),
        crate::DType::BF16,
        "gelu_exact_bf16_iter expects BF16"
    );
    let mut iter = TensorIteratorBase::build_unary_op(None, x)?;
    gelu_exact_bf16_kernel(&mut iter)?;
    iter.take_output(0)
}
