...
use crate::{CudaDevice, DType, Error, Result, Shape, Tensor};
...
impl AutoencoderKL {
    ...
    pub fn new_random(device: CudaDevice, dtype: DType) -> Result<Self> {
        ...
        let w = |kh, kw, ic, oc| -> Result<Tensor> {
            ...
            match dtype {
                DType::BF16 => t.to_bf16(),
                DType::F32 => Ok(t),
                _ => Ok(t.to_dtype(dtype)?),
            }
        };
        Ok(Self {
            _device: dev.clone(),
    ...
    pub fn encode(&self, x: &Tensor) -> Result<Tensor> {
        ...
        if dims.len() != 4 || dims[3] != 3 {
            return Err(Error::InvalidOperation(
                "encode expects NHWC with 3 channels".into(),
            ));
        }
    ...
    pub fn decode(&self, z: &Tensor) -> Result<Tensor> {
        ...
        if dims.len() != 4 || dims[3] != 4 {
            return Err(Error::InvalidOperation(
                "decode expects NHWC with 4 channels".into(),
            ));
        }
        // Unscale
        ...
        for (w, _oc) in [
            (&self.dec_w1, 256usize),
            (&self.dec_w2, 128usize),
            (&self.dec_w3, 64usize),
        ] {
            let h = y.shape().dims()[1] * 2;
            let wout = y.shape().dims()[2] * 2;
