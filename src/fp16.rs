...
// Legacy mixed-precision infrastructure; pending Phase 3 verification.
...
use crate::{CudaDevice, DType, Error, Result, Tensor};
use half::{bf16, f16};
use std::sync::Arc;
...
pub fn cast_tensor(tensor: &Tensor, target_dtype: DType) -> Result<Tensor> {
    ...
    match target_dtype {
        DType::F16 => {
            ...
            let f32_data: Vec<f32> = fp16_data.iter().map(|&x| x.to_f32()).collect();
...
impl MixedPrecisionTraining {
    pub fn new(precision: Precision, initial_loss_scale: f32, device: Arc<CudaDevice>) -> Self {
        Self {
            precision,
...
impl<O> FP16Optimizer<O> {
    pub fn new(base_optimizer: O, params: Vec<Tensor>, device: Arc<CudaDevice>) -> Result<Self> {
    ...
    pub fn step(&mut self, gradients: Vec<Tensor>) -> Result<()> {
        ...
        if !found_overflow {
            ...
            for (i, (fp32_param, grad)) in self
                .fp32_params
                .iter_mut()
                .zip(unscaled_grads.iter())
                .enumerate()
            {
...
mod tests {
    ...
    fn test_fp16_conversion() -> Result<()> {
        ...
        let tensor = Tensor::randn(Shape::from_dims(&[2, 3, 4]), 0.0, 1.0, device)?;
    ...
    fn test_mixed_precision_scaler() -> Result<()> {
        ...
        let mut mp = MixedPrecisionTraining::new(Precision::FP16, 1024.0, device.clone());

        let loss = Tensor::from_vec(vec![0.5], Shape::from_dims(&[1]), device)?;
