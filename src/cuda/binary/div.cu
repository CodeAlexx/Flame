// flame-core/src/cuda/binary/div.cu
//
// Phase 5b — migrate div onto the TensorIterator pipeline. Mirrors
// src/cuda/binary/add.cu shape.
//
// Functor math: fp32 round-trip divide. Derived from
// src/bf16_elementwise.rs CUDA_ADD_MUL_BF16 div_bf16_kernel (L246):
//   O[tid] = __float2bfloat16(__bfloat162float(A[a_off]) / __bfloat162float(B[b_off]));
//
// Reference: PyTorch aten/src/ATen/native/cuda/BinaryDivTrueKernel.cu.
...
extern "C" int flame_div_bf16_kernel(
    ...
    void* stream_void)
{
    ...
    flame::iter::launch_gpu_kernel<2, DivBF16Op>(*meta, DivBF16Op{}, stream);
