// flash_attention_fwd_legacy.cu
// Preserved BQ=32 WMMA forward kernel for A/B parity testing with the new
// FA2-style tiled kernel (see flash_attention_fwd.cu). Exposes a separate
// FFI symbol `flame_flash_attention_bf16_wmma_legacy` so tests can compare
// outputs. Production callers should continue using flame_flash_attention_bf16,
// which is routed to the new kernel. This file will be deleted in Phase 3;
...
// Symbols are suffixed `_legacy` to avoid colliding with the new FA2 kernel
// (flash_attention_fwd.cu). Host entry is flame_flash_attention_bf16_wmma_legacy
// below.
...
// ============================================================================
__device__ __forceinline__ void load_tile_bf16_legacy(
...
__global__ void flash_attn_fwd_legacy_hd##HD(                                        \
    ...
    load_tile_bf16_legacy(s_Q, Q_base, q_rows, HD, BQ, HD, THREADS, tid);           \
                                                                              \
    ...
    for (int kv_t = 0; kv_t < num_kv_tiles; kv_t++) {                        \
        ...
        load_tile_bf16_legacy(s_K, K_base + (size_t)kv_start * HD,                   \
                       kv_rows, HD, BKV, HD, THREADS, tid);                   \
        load_tile_bf16_legacy(s_V, V_base + (size_t)kv_start * HD,                   \
                       kv_rows, HD, BKV, HD, THREADS, tid);                   \
        __syncthreads();                                                      \
                                                                              \
...
// ============================================================================
// Legacy entry point — preserved for A/B parity testing only.
// Callers in production continue to use flame_flash_attention_bf16 which is
// provided by the new FA2 kernel (flash_attention_fwd.cu).
// ============================================================================
extern "C" int flame_flash_attention_bf16_wmma_legacy(
    const void* Q,
    const void* K,
    const void* V,
    void* O,
    ...
    #define LAUNCH_FLASH_WMMA(HD) do {                                        \
        ...
        cudaError_t attr_err = cudaFuncSetAttribute(                          \
            flash_attn_fwd_legacy_hd##HD,                                            \
            cudaFuncAttributeMaxDynamicSharedMemorySize,                      \
            (int)smem_size                                                    \
        );                                                                    \
        if (attr_err != cudaSuccess) return (int)attr_err;                    \
        flash_attn_fwd_legacy_hd##HD<<<grid, block, smem_size, s>>>(                 \
