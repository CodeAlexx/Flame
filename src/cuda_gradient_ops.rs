...
extern "C" __global__ void check_invalid_kernel(
    const float* data,
    const int n,
    float* flag
) {
    ...
    if (idx < n) {
        float v = data[idx];
        if (isnan(v) || isinf(v)) {
            atomicExch(flag, 1.0f);
        }
    }
}

extern "C" __global__ void constrain_grads_kernel(
    float* data,
    const float max_abs,
    const int n
) {
...
impl CudaGradientOps {
    ...
    pub fn clip_gradient(&self, grad: &mut Tensor, clip_value: f32) -> Result<()> {
        ...
        let f = self
            ...
            .ok_or_else(|| Error::Cuda("Failed to get clip_gradient kernel".into()))?;
    ...
    pub fn compute_l2_norm(&self, tensor: &Tensor) -> Result<f32> {
        ...
        let f1 = self
            ...
            .ok_or_else(|| Error::Cuda("Failed to get compute_l2_norm kernel".into()))?;
        ...
        let f2 = self
            ...
            .ok_or_else(|| Error::Cuda("Failed to get finalize_l2_norm kernel".into()))?;
    ...
    pub fn add_gradient_noise(&self, grad: &mut Tensor, noise_scale: f32) -> Result<()> {
        ...
        let f = self
            ...
            .ok_or_else(|| Error::Cuda("Failed to get add_gradient_noise kernel".into()))?;
    ...
    pub fn scale_gradient(&self, grad: &mut Tensor, scale: f32) -> Result<()> {
        ...
        let f = self
            ...
            .ok_or_else(|| Error::Cuda("Failed to get scale_gradient kernel".into()))?;
    ...
    pub fn clamp_tensor(&self, tensor: &mut Tensor, min_val: f32, max_val: f32) -> Result<()> {
        ...
        let f = self
            ...
            .ok_or_else(|| Error::Cuda("Failed to get clamp_tensor kernel".into()))?;
    ...
    pub fn adam_update(
        ...
        let f = self
            ...
            .ok_or_else(|| Error::Cuda("Failed to get adam_update kernel".into()))?;
    ...
    /// Check tensor values for NaN/Inf on GPU
    pub fn has_invalid_values(&self, tensor: &Tensor) -> Result<bool> {
        let n = tensor.shape().elem_count();
        if n == 0 {
            return Ok(false);
        }
        ...
        let f = self
            .device
            .get_func("gradient_ops", "check_invalid_kernel")
            .ok_or_else(|| Error::Cuda("Failed to get check_invalid_kernel".into()))?;

        let cfg = LaunchConfig::for_num_elems(n as u32);
        let flag = self.device.alloc_zeros::<f32>(1)?;
        launch_kernel!(f, cfg, tensor.storage.try_as_slice_f32()?, n as i32, &flag)?;

        self.device.synchronize()?;
        let host: Vec<f32> = self.device.dtoh_sync_copy(&flag)?;
        Ok(host[0] > 0.5)
    }

    /// Clamp by absolute value and clear invalid entries on GPU
    pub fn constrain_by_abs(&self, tensor: &mut Tensor, max_abs: f32) -> Result<()> {
        let n = tensor.shape().elem_count();
        if n == 0 {
            return Ok(());
        }
        ...
        let f = self
            .device
            .get_func("gradient_ops", "constrain_grads_kernel")
            .ok_or_else(|| Error::Cuda("Failed to get constrain_grads_kernel".into()))?;
