...
impl Conv3d {
    ...
    fn conv3d_cuda(&self, input: &Tensor, output_shape: Shape) -> Result<Tensor> {
        ...
        let ptx = compile_ptx(&kernel_code)
            .map_err(|e| Error::Cuda(format!("Failed to compile conv3d kernel: {:?}", e)))?;
        self.device.load_ptx(ptx, "conv3d", &["conv3d_kernel"])
            .map_err(|e| Error::Cuda(format!("Failed to load conv3d kernel: {:?}", e)))?;
        
        let f = self.device.get_func("conv3d", "conv3d_kernel")
            .ok_or_else(|| Error::Cuda("Failed to get conv3d_kernel".into()))?;
    ...
    fn add_bias_3d(&self, output: &Tensor, bias: &Tensor) -> Result<Tensor> {
        ...
        let ptx = compile_ptx(&kernel_code)
            .map_err(|e| Error::Cuda(format!("Failed to compile add_bias_3d kernel: {:?}", e)))?;
        self.device.load_ptx(ptx, "add_bias_3d", &["add_bias_3d_kernel"])
            .map_err(|e| Error::Cuda(format!("Failed to load add_bias_3d kernel: {:?}", e)))?;
        
        let kernel = self.device.get_func("add_bias_3d", "add_bias_3d_kernel")
            .ok_or_else(|| Error::Cuda("Failed to get add_bias_3d_kernel".into()))?;
...
impl BatchNorm3d {
    ...
    fn apply_batch_norm_3d(&self, input: &Tensor, mean: &Tensor, var: &Tensor) -> Result<Tensor> {
        ...
        let ptx = compile_ptx(&kernel_code)
            .map_err(|e| Error::Cuda(format!("Failed to compile batch_norm_3d kernel: {:?}", e)))?;
        self.device.load_ptx(ptx, "batch_norm_3d", &["batch_norm_3d_kernel"])
            .map_err(|e| Error::Cuda(format!("Failed to load batch_norm_3d kernel: {:?}", e)))?;
        
        let kernel = self.device.get_func("batch_norm_3d", "batch_norm_3d_kernel")
            .ok_or_else(|| Error::Cuda("Failed to get batch_norm_3d_kernel".into()))?;
