    ...
    fn compute_gradients(
        ...
        match &entry.op {
            ...
            Op::AddBias { input, bias } => {
                ...
                for _ in 0..grad_shape.len() - 1 {
                    grad_bias = grad_bias.sum_dim(0)?;
                }

                Ok(vec![(*input, input_grad), (*bias, grad_bias)])
            }
...
extern "C" __global__ void relu_backward_kernel(
    ...
    if (idx < numel) {
        ...
        let f = input
            .device
            .get_func("relu_backward_kernel", "relu_backward_kernel")
            .ok_or_else(|| crate::Error::Cuda("Failed to get relu_backward kernel".into()))?;
...
thread_local! {
    static ENGINE: std::cell::RefCell<Option<AutogradEngine>> = const { std::cell::RefCell::new(None) };
}
