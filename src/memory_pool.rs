...
use crate::cuda_memory_alignment::align_size;
use crate::{Error, Result, Shape};
use cudarc::driver::{CudaDevice, CudaSlice};
use std::collections::{HashMap, VecDeque};
use std::sync::{Arc, Mutex};

fn poison<T>(_: std::sync::PoisonError<T>) -> Error {
    Error::Training("memory pool mutex poisoned".into())
}
...
impl DeviceMemoryPool {
    ...
    pub fn allocate(&mut self, size: usize) -> Result<CudaSlice<f32>> {
        ...
        let ptr = match self.device.alloc_zeros::<f32>(pool_size) {
            ...
            Err(_e) => return Err(Error::CudaDriver),
        };
        ...
        // Add to pool
        self.pools.entry(pool_size).or_default().push_back(block);
    ...
    pub fn force_cleanup(&self) -> Result<()> {
        ...
        self.device.synchronize().map_err(|_| Error::CudaDriver)?;
...
impl MemoryPoolManager {
    ...
    pub fn get_pool(&self, device: &Arc<CudaDevice>) -> Result<Arc<Mutex<DeviceMemoryPool>>> {
        ...
        let pool = pools
            ...
            .or_insert_with(|| Arc::new(Mutex::new(DeviceMemoryPool::new(device.clone(), None))))
            .clone();
    ...
    pub fn global_counters(&self) -> (usize, usize, f32) {
        ...
        let reuse_pct = if allocations > 0 {
            (reuses as f32) * 100.0 / (allocations as f32)
        } else {
            0.0
        };
...
pub fn allocate_pooled(shape: Shape, device: Arc<CudaDevice>) -> Result<PooledTensor> {
...
impl Workspace {
    ...
    pub fn get_buffer(&mut self, size: usize) -> Result<&CudaSlice<f32>> {
        ...
        self.buffers
            .last()
            .ok_or_else(|| Error::Training("memory pool empty".into()))
    }
