...
impl KernelLauncher {
    ...
    pub fn compile_kernel(
        ...
        {
            let cache = KERNEL_CACHE.lock().map_err(|_| crate::Error::Training("kernel launcher cache mutex poisoned".into()))?;
        ...
        let ptx = compile_ptx_with_opts(source_code, opts)
            .map_err(|e| Error::Cuda(format!("Kernel compilation failed: {:?}", e)))?;
        ...
        {
            let mut cache = KERNEL_CACHE.lock().map_err(|_| crate::Error::Training("kernel launcher cache mutex poisoned".into()))?;
