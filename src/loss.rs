...
// Legacy loss implementations awaiting modernization.
...
#[cfg(test)]
use crate::Shape;
use crate::{AutogradContext, Error, Op, Result, Tensor};
...
pub fn mse_loss_bf16(pred: &Tensor, target: &Tensor) -> Result<Tensor> {
    ...
    if pred.dtype() != DType::BF16 || target.dtype() != DType::BF16 {
        return Err(Error::InvalidInput(
            "mse_loss_bf16 expects BF16 inputs".into(),
        ));
    }
...
pub fn huber_loss(predictions: &Tensor, targets: &Tensor, delta: f32) -> Result<Tensor> {
    ...
    if delta <= 0.0 {
        return Err(Error::InvalidOperation(format!(
            "Huber loss delta must be positive, got {}",
            delta
        )));
    }
    ...
    let half_delta = Tensor::from_vec(
        half_delta_vec,
        abs_diff.shape.clone(),
        abs_diff.device.clone(),
    )?;
