...
// Legacy fused kernels kept for reference; pending modernization.
...
use cudarc::driver::{CudaFunction, LaunchAsync};
use std::sync::Arc;
...
"#;
        ...
        let f = x
            .device
            .get_func("bias_gelu_kernel", "bias_gelu_kernel")
            .ok_or_else(|| Error::Cuda("Failed to get bias_gelu_kernel".into()))?;
...
"#;
        ...
        let f = x
            .device
            .get_func("layernorm_linear_kernel", "layernorm_linear_kernel")
            .ok_or_else(|| Error::Cuda("Failed to get layernorm_linear_kernel".into()))?;
        ...
        let bias_to_use = if let Some(b) = bias {
            ...
            dummy_bias =
                Tensor::zeros(Shape::from_dims(&[out_features as usize]), x.device.clone())?;
            &dummy_bias
        };
...
"#;
        ...
        let f = input
            .device
            .get_func("gelu_backward_kernel", "gelu_backward_kernel")
            .ok_or_else(|| Error::Cuda("Failed to get gelu_backward_kernel".into()))?;
...
"#;
        ...
        let f = param
            .device
            .get_func("adam_step_kernel", "adam_step_kernel")
            .ok_or_else(|| Error::Cuda("Failed to get adam_step_kernel".into()))?;
...
mod tests {
    ...
    fn test_bias_gelu() -> Result<()> {
        ...
        let bias = Tensor::randn(Shape::from_dims(&[hidden_size]), 0.0, 0.01, device.clone())?;
