...
impl CudaTensor {
    ...
    pub fn randn(shape: Shape, mean: f32, std: f32, device: Arc<CudaDevice>) -> Result<Self> {
        ...
        let normal = Normal::new(mean, std)
            .map_err(|e| Error::InvalidOperation(format!("invalid normal params: {e}")))?;

        let cpu_data: Vec<f32> = (0..size).map(|_| normal.sample(&mut rng)).collect();
    ...
    pub fn update_weights(&mut self, gradient: &CudaTensor, lr: f32) -> Result<()> {
        ...
        if let crate::tensor_storage::TensorStorage::F32 { data, .. } = &updated.storage {
            self.data = slice_ref(data).clone();
        } else {
            return Err(Error::InvalidOperation("Expected F32 storage".into()));
        }
        Ok(())
    }
    ...
    pub fn matmul(&self, other: &CudaTensor) -> Result<CudaTensor> {
        let (m, k) = match self.shape.dims() {
            [m, k] => (*m, *k),
            _ => return Err(Error::InvalidOperation("matmul requires 2D tensors".into())),
        };
        ...
        let blas = CudaBlas::new(self.device.clone()).map_err(|_| Error::CuBlas)?;
    ...
    pub fn add(&self, other: &CudaTensor) -> Result<CudaTensor> {
        ...
        if self.shape == other.shape {
            ...
            if let crate::tensor_storage::TensorStorage::F32 { data, .. } = &result.storage {
                output.data = slice_ref(data).clone();
            } else {
                return Err(Error::InvalidOperation("Expected F32 storage".into()));
            }
        } else {
            return Err(Error::InvalidOperation(
                "Broadcasting not implemented yet".into(),
            ));
        }
    ...
    pub fn mul(&self, other: &CudaTensor) -> Result<CudaTensor> {
        if self.shape != other.shape {
            return Err(Error::InvalidOperation(
                "Shapes must match for multiplication".into(),
            ));
        }
    ...
    pub fn sum(&self) -> Result<CudaTensor> {
        ...
        let _total_elements = self.shape.elem_count();

        // Allocate output buffer for single element
        let _sum_buf = unsafe { self.device.alloc::<f32>(1)? };
        ...
        if let crate::tensor_storage::TensorStorage::F32 { data, .. } = &sum_result.storage {
            Ok(CudaTensor {
                data: slice_ref(data).clone(),
                shape: sum_result.shape.clone(),
                device: sum_result.device.clone(),
            })
        } else {
            Err(Error::InvalidOperation("Expected F32 storage".into()))
        }
    }
