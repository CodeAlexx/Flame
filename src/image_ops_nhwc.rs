...
pub fn resize_bilinear_nhwc(
    ...
    if dt != DType::F32 && dt != DType::BF16 {
        return Err(Error::InvalidOperation(format!(
            "resize_bilinear_nhwc: unsupported dtype {:?} (expect F32/BF16)",
            dt
        )));
    }
...
pub fn normalize_nhwc(x: &Tensor, mean: &[f32], std: &[f32]) -> Result<Tensor> {
    ...
    if mean.len() != c || std.len() != c {
        return Err(Error::InvalidOperation(format!(
            "normalize_nhwc: mean/std must each have C={} elements",
            c
        )));
    }
    let dt = x.dtype();
    if dt != DType::F32 && dt != DType::BF16 {
        return Err(Error::InvalidOperation(format!(
            "normalize_nhwc: unsupported dtype {:?} (expect F32/BF16)",
            dt
        )));
    }
