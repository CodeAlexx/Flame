...
#[no_mangle]
pub extern "C" fn flame_zeros_nhwc_bf16(
    dev: *mut Device,
    b: i32,
    h: i32,
    w: i32,
    ...
    let d = unsafe { &*dev };
    let res = Tensor::zeros(
        Shape::from_dims(&[b as usize, h as usize, w as usize, c as usize]),
        d.cuda_device().clone(),
    )
...
#[no_mangle]
pub extern "C" fn flame_tensor_free(p: *mut Tensor) {
    if !p.is_null() {
        unsafe {
            drop(Box::from_raw(p));
        }
