...
// Legacy CUDA kernel utilities; modern replacements live in ops::conv2d.
...
lazy_static::lazy_static! {
    ...
    static ref CONV2D_MODULE: Result<()> = Err(Error::Cuda("Conv2D module not compiled".into()));
}
...
const FLOOR_KERNEL: &str = "floor_kernel";
const CEIL_KERNEL: &str = "ceil_kernel";
const ROUND_KERNEL: &str = "round_kernel";
const UPDATE_WEIGHTS_KERNEL: &str = "update_weights_kernel";
const SUM_ALL_F32_KERNEL: &str = "sum_all_kernel";
const SUM_ALL_BF16_KERNEL: &str = "sum_all_bf16_kernel";
...
const GATHER_ROWS_F32_KERNEL: &str = "gather_rows_f32_kernel";
const GATHER_ROWS_BF16_KERNEL: &str = "gather_rows_bf16_kernel";
const SCATTER_ADD_ROWS_F32_KERNEL: &str = "scatter_add_rows_f32_kernel";
...
pub fn compile_kernel(name: &str, code: &str) -> Result<cudarc::nvrtc::Ptx> {
    ...
    compile_ptx_with_opts(code, opts)
        .map_err(|e| Error::Cuda(format!("CUDA compilation failed for {}: {:?}", name, e)))
}
    ...
    pub fn gather_rows(input: &Tensor, indices: &Tensor, dim: usize) -> Result<Tensor> {
        if dim != 0 {
            return Err(Error::Unsupported(
                "gather_rows currently supports dim=0 only".into(),
            ));
        }
        ...
        if input_dims.is_empty() {
            return Err(Error::InvalidOperation(
                "gather_rows requires rank >= 1".into(),
            ));
        }

        let rows = input_dims[0];
        let tail_dims = &input_dims[1..];
        let row_elems: usize = if tail_dims.is_empty() {
            1
        } else {
            tail_dims.iter().product()
        };
        ...
        let indices_flat = indices_f32.flatten_all()?;
        let k = indices_flat.shape().elem_count();
        if k == 0 {
            return Ok(output);
        }
        ...
        let threads: u32 = 256;
        let blocks_y = ((row_elems as u32 + threads - 1) / threads).max(1);
        let cfg = LaunchConfig {
            grid_dim: (k as u32, blocks_y, 1),
        ...
        match input.dtype() {
            DType::F32 => {
                let kernel_code = r#"
extern "C" __global__ void gather_rows_f32_kernel(
    ...
    long long rows)
{
    ...
    long long j = blockIdx.y * blockDim.x + threadIdx.x;
    if (j >= row_elems) return;
    int r = static_cast<int>(idx[i]);
    if (r < 0 || r >= rows) return;
    y[i * row_elems + j] = x[(long long)r * row_elems + j];
}
"#;

                CudaKernels::ensure_kernel(device, GATHER_ROWS_F32_KERNEL, kernel_code)?;
                let func = device
                    .get_func(GATHER_ROWS_F32_KERNEL, GATHER_ROWS_F32_KERNEL)
                    .ok_or_else(|| Error::Cuda("Failed to load gather_rows_f32_kernel".into()))?;

                unsafe {
                    launch_kernel!(
                        func,
                        cfg,
                        input.storage_ref().try_as_slice_f32()?,
                        indices_flat.storage.try_as_slice_f32()?,
            ...
            DType::BF16 => {
                let kernel_code = r#"
extern "C" __global__ void gather_rows_bf16_kernel(
...
"#;
                ...
                CudaKernels::ensure_kernel(device, GATHER_ROWS_BF16_KERNEL, kernel_code)?;
                let func = device
                    .get_func(GATHER_ROWS_BF16_KERNEL, GATHER_ROWS_BF16_KERNEL)
                    .ok_or_else(|| Error::Cuda("Failed to load gather_rows_bf16_kernel".into()))?;

                #[cfg(feature = "bf16_u16")]
                unsafe {
                    // println!("DEBUG: gather_rows_bf16_kernel launch");
                    let input_ptr = input.as_device_ptr_bf16("gather_rows_input")? as u64;
                    let output_ptr = output.as_mut_device_ptr_bf16("gather_rows_output")? as u64;
                ...
                {
                    return Err(Error::Unsupported(
                        "gather_rows BF16 requires bf16_u16 feature".into(),
                    ));
                }
            }
            other => {
                return Err(Error::Unsupported(format!(
                    "gather_rows does not support dtype {:?}",
                    other
                )));
            }
        }
...
extern "C" __global__ void sum_dim_keepdim_kernel(
    ...
    output[tid] = sum;
        ...
        let f = tensor
            .device
            .get_func("sum_dim_keepdim_kernel", "sum_dim_keepdim_kernel")
            .ok_or_else(|| Error::Cuda("Failed to get sum_dim_keepdim_kernel".into()))?;
    ...
    /// Sum all elements in a tensor (supports F32 and BF16 storage)
    pub fn sum_all(&self, tensor: &Tensor) -> Result<Tensor> {
        match tensor.dtype() {
            DType::F32 => self.sum_all_f32(tensor),
            DType::BF16 => self.sum_all_bf16(tensor),
            other => Err(Error::Unsupported(format!(
                "sum_all: dtype {:?} not supported",
                other
            ))),
        }
    }

    fn sum_all_f32(&self, tensor: &Tensor) -> Result<Tensor> {
        let kernel_code = r#"
extern "C" __global__ void sum_all_kernel(
    ...
    if (tid == 0) {
        ...
        Self::ensure_kernel(&tensor.device, SUM_ALL_F32_KERNEL, kernel_code)?;

        let f = tensor
            .device
            .get_func(SUM_ALL_F32_KERNEL, SUM_ALL_F32_KERNEL)
        ...
        let block_size = 256;
        let grid_size = n.div_ceil(block_size).min(1024) as u32;
        let cfg = LaunchConfig {
    ...
    fn sum_all_bf16(&self, tensor: &Tensor) -> Result<Tensor> {
        #[cfg(not(feature = "bf16_u16"))]
        {
            return Err(Error::Unsupported(
                "sum_all_bf16 requires the bf16_u16 feature".into(),
            ));
        }
        ...
        #[cfg(feature = "bf16_u16")]
        {
            let kernel_code = r#"
#include <cuda_bf16.h>
extern "C" __global__ void sum_all_bf16_kernel(
    ...
    unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
    float sum = 0.0f;
    const __nv_bfloat16* src = reinterpret_cast<const __nv_bfloat16*>(input);
    while (i < n) {
...
"#;
            ...
            Self::ensure_kernel(&tensor.device, SUM_ALL_BF16_KERNEL, kernel_code)?;

            let f = tensor
                .device
                .get_func(SUM_ALL_BF16_KERNEL, SUM_ALL_BF16_KERNEL)
    ...
    pub(crate) fn ensure_kernel(
        ...
        device
            ...
            .map_err(|e| {
                Error::Cuda(format!("Failed to load kernel '{}': {:?}", kernel_name, e))
            })?;
    ...
    pub fn add(a: &Tensor, b: &Tensor) -> Result<Tensor> {
        let a_f32 = if a.dtype() == crate::DType::F32 {
            a.clone_result()?
        } else {
            a.to_dtype(crate::DType::F32)?
        };
        let b_f32 = if b.dtype() == crate::DType::F32 {
            b.clone_result()?
        } else {
            b.to_dtype(crate::DType::F32)?
        };
        // Support broadcasting: upcast operands to a common target shape
        let (a_t, b_t) = if a_f32.shape != b_f32.shape {
            let target = a_f32.shape.broadcast_shape_binary_op(&b_f32.shape)?;
            let a_bc = if a_f32.shape != target {
                Self::broadcast(&a_f32, &target)?
            } else {
                a_f32.clone_result()?
            };
            let b_bc = if b_f32.shape != target {
                Self::broadcast(&b_f32, &target)?
            } else {
                b_f32.clone_result()?
            };
            (a_bc, b_bc)
...
extern "C" __global__ void add_kernel(float *out, const float *a, const float *b, int numel) {
    ...
    if (idx < numel) {
        ...
        Self::ensure_kernel(&a_t.device, ADD_KERNEL, kernel_code)?;

        let f = a_t
            .device
            .get_func(ADD_KERNEL, ADD_KERNEL)
            .ok_or_else(|| Error::Cuda("Failed to get add_kernel".into()))?;

        let numel = a_t.shape.elem_count();

        // Allocate output data
        let mut output_data = crate::tensor::alloc_from_pool(&a_t.device, numel)
            .map_err(|e| Error::CudaDriver(format!("{e:?}")))?;
    ...
    pub fn mul(a: &Tensor, b: &Tensor) -> Result<Tensor> {
        ...
        let (a_t, b_t) = if a.shape != b.shape {
            ...
            let a_bc = if a.shape != target {
                Self::broadcast(a, &target)?
            } else {
                a.clone_result()?
            };
            let b_bc = if b.shape != target {
                Self::broadcast(b, &target)?
            } else {
                b.clone_result()?
            };
            (a_bc, b_bc)
...
extern "C" __global__ void mul_kernel(float *out, const float *a, const float *b, int numel) {
    ...
    if (idx < numel) {
        ...
        Self::ensure_kernel(&a_t.device, MUL_KERNEL, kernel_code)?;

        let f = a_t
            .device
            .get_func(MUL_KERNEL, MUL_KERNEL)
            .ok_or_else(|| Error::Cuda("Failed to get mul_kernel".into()))?;
...
extern "C" __global__ void mul_scalar_kernel(float *out, const float *input, float scalar, int numel) {
    ...
    if (idx < numel) {
        ...
        let f = tensor
            .device
            .get_func(MUL_SCALAR_KERNEL, MUL_SCALAR_KERNEL)
            .ok_or_else(|| Error::Cuda("Failed to get mul_scalar_kernel".into()))?;
...
extern "C" __global__ void add_scalar_kernel(float *out, const float *input, float scalar, int numel) {
    ...
    if (idx < numel) {
        ...
        let f = tensor
            .device
            .get_func(ADD_SCALAR_KERNEL, ADD_SCALAR_KERNEL)
            .ok_or_else(|| Error::Cuda("Failed to get add_scalar_kernel".into()))?;
...
extern "C" __global__ void relu_kernel(float *out, const float *input, int numel) {
    ...
    if (idx < numel) {
        ...
        let f = tensor
            .device
            .get_func(RELU_KERNEL, RELU_KERNEL)
            .ok_or_else(|| Error::Cuda("Failed to get relu_kernel".into()))?;
    ...
    pub fn floor(tensor: &Tensor) -> Result<Tensor> {
        let kernel_code = r#"
extern "C" __global__ void floor_kernel(float *out, const float *input, int numel) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < numel) {
        out[idx] = floorf(input[idx]);
    }
}"#;

        Self::ensure_kernel(&tensor.device, FLOOR_KERNEL, kernel_code)?;

        let f = tensor
            .device
            .get_func(FLOOR_KERNEL, FLOOR_KERNEL)
            .ok_or_else(|| Error::Cuda("Failed to get floor_kernel".into()))?;

        let numel = tensor.shape.elem_count();
        let mut output_data = crate::tensor::alloc_from_pool(&tensor.device, numel)?;
    ...
    pub fn ceil(tensor: &Tensor) -> Result<Tensor> {
        let kernel_code = r#"
extern "C" __global__ void ceil_kernel(float *out, const float *input, int numel) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < numel) {
        out[idx] = ceilf(input[idx]);
    }
}"#;

        Self::ensure_kernel(&tensor.device, CEIL_KERNEL, kernel_code)?;

        let f = tensor
            .device
            .get_func(CEIL_KERNEL, CEIL_KERNEL)
            .ok_or_else(|| Error::Cuda("Failed to get ceil_kernel".into()))?;
    ...
    pub fn round(tensor: &Tensor) -> Result<Tensor> {
        let kernel_code = r#"
extern "C" __global__ void round_kernel(float *out, const float *input, int numel) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < numel) {
        out[idx] = roundf(input[idx]);
    }
}"#;

        Self::ensure_kernel(&tensor.device, ROUND_KERNEL, kernel_code)?;

        let f = tensor
            .device
            .get_func(ROUND_KERNEL, ROUND_KERNEL)
            .ok_or_else(|| Error::Cuda("Failed to get round_kernel".into()))?;
...
extern "C" __global__ void gelu_kernel(float *out, const float *input, int numel) {
    ...
    if (idx < numel) {
        ...
        let f = tensor
            .device
            .get_func(GELU_KERNEL, GELU_KERNEL)
            .ok_or_else(|| Error::Cuda("Failed to get gelu_kernel".into()))?;
...
extern "C" __global__ void silu_kernel(float *out, const float *input, int numel) {
    ...
    if (idx < numel) {
        ...
        let f = tensor
            .device
            .get_func(SILU_KERNEL, SILU_KERNEL)
            .ok_or_else(|| Error::Cuda("Failed to get silu_kernel".into()))?;
...
extern "C" __global__ void tanh_kernel(float *out, const float *input, int numel) {
    ...
    if (idx < numel) {
        ...
        let f = tensor
            .device
            .get_func(TANH_KERNEL, TANH_KERNEL)
            .ok_or_else(|| Error::Cuda("Failed to get tanh_kernel".into()))?;
...
extern "C" __global__ void sigmoid_kernel(float *out, const float *input, int numel) {
    ...
    if (idx < numel) {
        ...
        let f = tensor
            .device
            .get_func(SIGMOID_KERNEL, SIGMOID_KERNEL)
            .ok_or_else(|| Error::Cuda("Failed to get sigmoid_kernel".into()))?;
    ...
    /// Sum kernel wrapper (kept for compatibility)
...
extern "C" __global__ void transpose_kernel(
    ...
    if (x < height && y < width) {
        ...
        let f = tensor
            .device
            .get_func(TRANSPOSE_KERNEL, TRANSPOSE_KERNEL)
            .ok_or_else(|| Error::Cuda("Failed to get transpose_kernel".into()))?;
        ...
        let grid_y = rows.div_ceil(tile_size);
...
extern "C" __global__ void leaky_relu_kernel(float *out, const float *input, float negative_slope, int numel) {
    ...
    if (idx < numel) {
        ...
        let f = tensor
            .device
            .get_func("leaky_relu_kernel", "leaky_relu_kernel")
            .ok_or_else(|| Error::Cuda("Failed to get leaky_relu_kernel".into()))?;
...
extern "C" __global__ void elu_kernel(float *out, const float *input, float alpha, int numel) {
    ...
    if (idx < numel) {
        ...
        let f = tensor
            .device
            .get_func("elu_kernel", "elu_kernel")
            .ok_or_else(|| Error::Cuda("Failed to get elu_kernel".into()))?;
...
extern "C" __global__ void prelu_kernel(
    ...
    if (idx < numel) {
        ...
        let f = tensor
            .device
            .get_func("prelu_kernel", "prelu_kernel")
            .ok_or_else(|| Error::Cuda("Failed to get prelu_kernel".into()))?;

        let numel = tensor.shape.elem_count();
        let shape_dims = tensor.shape.dims();
        let num_channels = if shape_dims.len() >= 2 {
            shape_dims[1]
        } else {
            1
        };
...
extern "C" __global__ void maxpool2d_with_indices_kernel(
    ...
    if (idx < total_output) {
        ...
        let f = input
            .device
            .get_func(
                "maxpool2d_with_indices_kernel",
                "maxpool2d_with_indices_kernel",
            )
            .ok_or_else(|| Error::Cuda("Failed to get maxpool2d_kernel".into()))?;
        ...
        let (batch, channels, h_in, w_in) = match dims {
            ...
            _ => {
                return Err(Error::InvalidOperation(
                    "MaxPool2d requires 4D input".into(),
                ))
            }
        };
    ...
    pub fn maxpool2d_forward(
        ...
        let (output, _indices) =
            Self::maxpool2d_forward_with_indices(input, kernel_size, stride, padding)?;
        Ok(output)
    }
...
extern "C" __global__ void maxpool2d_backward_with_indices_kernel(
    ...
    if (idx < total_output) {
        ...
        Self::ensure_kernel(
            &input.device,
            "maxpool2d_backward_with_indices_kernel",
            kernel_code,
        )?;
        ...
        let f = input
            ...
            .ok_or_else(|| {
                Error::Cuda("Failed to get maxpool2d_backward_with_indices_kernel".into())
            })?;
...
extern "C" __global__ void avgpool2d_kernel(
    ...
    if (idx < total_output) {
        ...
        let f = input
            .device
            .get_func("avgpool2d_kernel", "avgpool2d_kernel")
            .ok_or_else(|| Error::Cuda("Failed to get avgpool2d_kernel".into()))?;
        ...
        let (batch, channels, h_in, w_in) = match dims {
            ...
            _ => {
                return Err(Error::InvalidOperation(
                    "AvgPool2d requires 4D input".into(),
                ))
            }
        };
...
extern "C" __global__ void avgpool2d_backward_kernel(
    ...
    if (idx < total_input) {
        ...
        let f = input
            .device
            .get_func("avgpool2d_backward_kernel", "avgpool2d_backward_kernel")
            .ok_or_else(|| Error::Cuda("Failed to get avgpool2d_backward_kernel".into()))?;
        ...
        let (batch, channels, h_in, w_in) = match dims {
            ...
            _ => {
                return Err(Error::InvalidOperation(
                    "AvgPool2d backward requires 4D input".into(),
                ))
            }
        };
        ...
        let (_, _, h_out, w_out) = match grad_dims {
            ...
            _ => {
                return Err(Error::InvalidOperation(
                    "AvgPool2d backward requires 4D grad_output".into(),
                ))
            }
        };
...
extern "C" __global__ void adaptive_maxpool2d_kernel(
    ...
    if (idx < total_output) {
        ...
        let (batch, channels, h_in, w_in) = match dims {
            ...
            _ => {
                return Err(Error::InvalidOperation(
                    "AdaptiveMaxPool2d requires 4D input".into(),
                ))
            }
        };
        ...
        let f = input
            .device
            .get_func("adaptive_maxpool2d_kernel", "adaptive_maxpool2d_kernel")
            .ok_or_else(|| Error::Cuda("Failed to get adaptive_maxpool2d_kernel".into()))?;
...
extern "C" __global__ void adaptive_avgpool2d_kernel(
    ...
    if (idx < total_output) {
        ...
        let (batch, channels, h_in, w_in) = match dims {
            ...
            _ => {
                return Err(Error::InvalidOperation(
                    "AdaptiveAvgPool2d requires 4D input".into(),
                ))
            }
        };
        ...
        let f = input
            .device
            .get_func("adaptive_avgpool2d_kernel", "adaptive_avgpool2d_kernel")
            .ok_or_else(|| Error::Cuda("Failed to get adaptive_avgpool2d_kernel".into()))?;
...
extern "C" __global__ void upsample2d_nearest_kernel(
    ...
    if (idx < total_output) {
        ...
        let f = input
            .device
            .get_func("upsample2d_nearest_kernel", "upsample2d_nearest_kernel")
            .ok_or_else(|| Error::Cuda("Failed to get upsample2d_nearest_kernel".into()))?;
        ...
        let (batch, channels, h_in, w_in) = match dims {
            ...
            _ => {
                return Err(Error::InvalidOperation(
                    "Upsample2d requires 4D input".into(),
                ))
            }
        };
...
extern "C" __global__ void upsample2d_bilinear_kernel(
    ...
    if (idx < total_output) {
        ...
        let f = input
            .device
            .get_func("upsample2d_bilinear_kernel", "upsample2d_bilinear_kernel")
            .ok_or_else(|| Error::Cuda("Failed to get upsample2d_bilinear_kernel".into()))?;
    ...
    pub fn conv_transpose2d_forward(
        ...
        let (batch, in_channels, h_in, w_in) = match dims {
            ...
            _ => {
                return Err(Error::InvalidOperation(
                    "ConvTranspose2d requires 4D input".into(),
                ))
            }
        };
        ...
        let (in_ch_w, out_channels, kh, kw) = match weight_dims {
            ...
            _ => {
                return Err(Error::InvalidOperation(
                    "ConvTranspose2d requires 4D weight".into(),
                ))
            }
        };
        ...
        if in_channels != in_ch_w {
            return Err(Error::InvalidOperation(format!(
                "Input channels {} doesn't match weight {}",
                in_channels, in_ch_w
            )));
        }
        ...
        Err(Error::KernelError(
            "Transposed convolution GPU kernel not yet implemented".into(),
        ))
    }
    ...
    pub fn conv_transpose2d_backward(
        ...
        Err(Error::KernelError(
            "Transposed convolution backward not yet implemented".into(),
        ))
    }

    /// Broadcast tensor to a new shape
    pub fn broadcast(input: &Tensor, target_shape: &Shape) -> Result<Tensor> {
        if std::env::var("FLAME_BC_TRACE").ok().as_deref() == Some("1") {
            eprintln!(
                "[bc-kernel] incoming dtype {:?} shape {:?} -> {:?}",
                input.dtype(),
                input.shape().dims().to_vec(),
                target_shape.dims().to_vec()
            );
        }
...
extern "C" __global__ void broadcast_kernel(
    ...
    output[idx] = input[src_idx];
        ...
        let f = input
            .device
            .get_func("broadcast_kernel", "broadcast_kernel")
            .ok_or_else(|| Error::Cuda("Failed to get broadcast_kernel".into()))?;
        ...
        // Calculate strides
        let src_strides: Vec<i32> = calculate_strides(src_shape)
            .iter()
            .map(|&x| x as i32)
            .collect();
        let dst_strides: Vec<i32> = calculate_strides(dst_shape)
            .iter()
            .map(|&x| x as i32)
            .collect();

        // Upload to device
        let src_shape_gpu = alloc_from_pool_and_copy(
            &input.device,
        ...
        let out = create_output_tensor(output_data, target_shape.clone(), input.device.clone());

        if src_dtype != DType::F32 {
            Ok(out.to_dtype(src_dtype)?)
        } else {
            Ok(out)
        }
    }

    /// Elementwise multiply with stride-based broadcasting (GPU, no materialization)
    pub fn mul_bc(a: &Tensor, b: &Tensor) -> Result<Tensor> {
        let ad: Vec<i32> = a.shape().dims().iter().map(|&x| x as i32).collect();
        let bd: Vec<i32> = b.shape().dims().iter().map(|&x| x as i32).collect();
        let as_: Vec<i32> = calculate_strides(a.shape().dims())
            .iter()
            .map(|&x| x as i32)
            .collect();
        let bs: Vec<i32> = calculate_strides(b.shape().dims())
            .iter()
            .map(|&x| x as i32)
            .collect();
        let tr = ad.len().max(bd.len());
        // Scalar×scalar: defer to equal-shape path
        if tr == 0 {
            return Self::mul(a, b);
        }
        let mut adp = vec![1i32; tr];
        let mut bdp = vec![1i32; tr];
        let mut asp = vec![0i32; tr];
        let mut bsp = vec![0i32; tr];
        for i in 0..ad.len() {
            adp[tr - ad.len() + i] = ad[i];
        }
        for i in 0..bd.len() {
            bdp[tr - bd.len() + i] = bd[i];
        }
        for i in 0..as_.len() {
            asp[tr - as_.len() + i] = as_[i];
        }
        for i in 0..bs.len() {
            bsp[tr - bs.len() + i] = bs[i];
        }
        let mut out: Vec<i32> = Vec::with_capacity(tr);
        let mut ao: Vec<i32> = Vec::with_capacity(tr);
        let mut bo: Vec<i32> = Vec::with_capacity(tr);
        for i in 0..tr {
            let (ai, bi) = (adp[i], bdp[i]);
            let od = if ai == bi {
                ai
            } else if ai == 1 {
                bi
            ...
            out.push(od);
            ao.push(if adp[i] == 1 { 0 } else { asp[i] });
            bo.push(if bdp[i] == 1 { 0 } else { bsp[i] });
        }
        let total: i32 = out.iter().product();

        let code = r#"
extern "C" __global__
void mul_bc_kernel(const float* __restrict__ A, const float* __restrict__ B,
                   float* __restrict__ O,
                   const float* __restrict__ out_dims,
                   const float* __restrict__ a_strides,
                   const float* __restrict__ b_strides,
                   int ndims, int total) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    int grid = gridDim.x * blockDim.x;
    if (ndims == 0) {
        if (tid == 0) { O[0] = A[0] * B[0]; }
        return;
    }
    while (tid < total) {
        int rem = tid;
        ...
        for (int i = ndims - 1; i >= 0; --i) {
            int d = (int)out_dims[i];
            int idx = rem % d; rem /= d;
            a_off += idx * (int)a_strides[i];
            b_off += idx * (int)b_strides[i];
        }
        O[tid] = A[a_off] * B[b_off];
        tid += grid;
    }
}
"#;
        Self::ensure_kernel(&a.device, "mul_bc_kernel", code)?;
        let f = a
            .device
            .get_func("mul_bc_kernel", "mul_bc_kernel")
            .ok_or_else(|| Error::Cuda("Failed to get mul_bc_kernel".into()))?;

        // Upload metadata using local helper (matches existing broadcast helpers)
        let out_gpu = alloc_from_pool_and_copy(&a.device, &out)?;
        let a_str_gpu = alloc_from_pool_and_copy(&a.device, &ao)?;
        ...
        unsafe {
            launch_kernel!(
                ...
                &b_str_gpu,
                i32::try_from(tr)
                    .map_err(|_| Error::InvalidOperation("rank exceeds i32".into()))?,
                total
            )?;
        }
        Ok(output)
    ...
    /// Elementwise add with stride-based broadcasting (GPU)
    pub fn add_bc(a: &Tensor, b: &Tensor) -> Result<Tensor> {
        let a = if a.dtype() == crate::DType::F32 {
            a.clone_result()?
        } else {
            a.to_dtype(crate::DType::F32)?
        };
        let b = if b.dtype() == crate::DType::F32 {
            b.clone_result()?
        } else {
            b.to_dtype(crate::DType::F32)?
        };
        ...
        let code = r#"
extern "C" __global__
void add_bc_kernel(const float* __restrict__ A, const float* __restrict__ B,
                   float* __restrict__ O,
                   const float* __restrict__ out_dims,
                   const float* __restrict__ a_strides,
                   const float* __restrict__ b_strides,
...
"#;
        Self::ensure_kernel(&a.device, "add_bc_kernel", code)?;
        let f = a
            .device
            .get_func("add_bc_kernel", "add_bc_kernel")
            .ok_or_else(|| Error::Cuda("Failed to get add_bc_kernel".into()))?;
        ...
        let ndims = i32::try_from(tr)
            .map_err(|_| Error::InvalidOperation("broadcast rank exceeds i32".into()))?;
        unsafe {
            launch_kernel!(
                f,
                cfg,
    ...
    pub fn scatter_add(
        ...
        if dim != 0 {
            return Err(Error::Unsupported(
                "scatter_add currently supports dim=0 only".into(),
            ));
        }

        let device = grad_output.device();
        let rows = input_shape[0];
        let tail = &input_shape[1..];
        let row_elems: usize = if tail.is_empty() {
            1
        } else {
            tail.iter().product()
        };

        let grad_f32 = if grad_output.dtype() == DType::F32 {
        ...
        let mut dst =
            Tensor::zeros_dtype(Shape::from_dims(input_shape), DType::F32, device.clone())?;

        if rows == 0 || row_elems == 0 || k == 0 {
            return Ok(dst);
        }

        let kernel_code = r#"
extern "C" __global__ void scatter_add_rows_f32_kernel(
    ...
    long long rows)
{
    ...
    int r = static_cast<int>(idx[i]);
    if (r < 0 || r >= rows) return;
    atomicAdd(&dst[(long long)r * row_elems + j], grad[i * row_elems + j]);
}
"#;

        CudaKernels::ensure_kernel(device, SCATTER_ADD_ROWS_F32_KERNEL, kernel_code)?;
        let func = device
            .get_func(SCATTER_ADD_ROWS_F32_KERNEL, SCATTER_ADD_ROWS_F32_KERNEL)
            .ok_or_else(|| Error::Cuda("Failed to load scatter_add_rows_f32_kernel".into()))?;
        ...
        unsafe {
            launch_kernel!(
                func,
                cfg,
                grad_f32.storage.try_as_slice_f32()?,
                indices_flat.storage.try_as_slice_f32()?,
                dst.storage_mut().try_as_mut_slice_f32()?,
                row_elems as i64,
                rows as i64
            )?;
        }
