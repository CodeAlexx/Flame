...
"###;
        ...
        let ptx = cudarc::nvrtc::compile_ptx(kernel_code)
            .map_err(|e| Error::Cuda(format!("Failed to compile conv2d kernel: {:?}", e)))?;

        // Choose kernel based on configuration
        let kernel_name = if kernel_h == 3
            && kernel_w == 3
            && stride_h == 1
            && stride_w == 1
            && pad_h == 1
        ...
        let num_blocks = total_threads.div_ceil(threads_per_block);
        ...
        let bias_slice = if let Some(b) = bias {
            ...
            zero_bias.as_ref().ok_or_else(|| {
                Error::Training("conv2d_fast: failed to allocate zero bias".into())
            })?
        };
        ...
        let kernel_func = device
            .get_func("conv2d_fast", kernel_name)
...
pub fn conv2d_fast(
    ...
    CudaConv2dFast::conv2d_forward(input, weight, bias, (stride, stride), (padding, padding), 1)
}
