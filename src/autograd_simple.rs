...
use crate::{Tensor, Result, Error, Shape};

// NOTE: `autograd_simple` is used as a rank-agnostic debug engine throughout the
// codebase (1D scalars, 2D matmuls, broadcasted tensors, etc.). Enforcing the
// NHWC/BF16 public-boundary contract here would require a redesign of every op
// that depends on arbitrary tensor ranks, so no boundary guards or dtype clamps
        ...
        for entry in self.tape.iter().rev() {
            ...
            match &entry.op {
                Op::Add { lhs, rhs, lhs_shape, rhs_shape } => {
                    ...
                    let lhs_grad = self.get_or_create_grad(*lhs, lhs_shape, device)?;
                    *lhs_grad = lhs_grad.add(&grad_output)?;
                    let rhs_grad = self.get_or_create_grad(*rhs, rhs_shape, device)?;
                ...
                Op::Sub { lhs, rhs } => {
                    ...
                    let lhs_shape = entry
                        ...
                        .get(lhs)
                        .map(|t| t.shape())
                        .unwrap_or_else(|| grad_output.shape());
                    let lhs_grad = self.get_or_create_grad(*lhs, lhs_shape, device)?;
                    *lhs_grad = lhs_grad.add(&grad_output)?;
                ...
                Op::MulScalar { input, scalar } => {
                    ...
                    let shape = entry
                        ...
                        .map(|t| t.shape())
                        .unwrap_or_else(|| grad_output.shape());
                    let input_grad = self.get_or_create_grad(*input, shape, device)?;
