...
impl CudaConv2dDirect {
    ...
    fn conv1x1_forward(input: &Tensor, weight: &Tensor, bias: Option<&Tensor>) -> Result<Tensor> {
...
"#;
        ...
        let ptx = cudarc::nvrtc::compile_ptx(kernel_src)
            .map_err(|e| Error::Cuda(format!("Failed to compile im2col_3x3 kernel: {:?}", e)))?;
        ...
        let kernel = device
            ...
            .ok_or_else(|| Error::Cuda("Failed to get kernel function".into()))?;

        // Launch kernel with optimal configuration
        let threads_per_block = 256;
        let total_pixels = batch_size * height * width;
        let pixels_per_thread = 4;
        let tiles = total_pixels.div_ceil(pixels_per_thread);
        let num_blocks = tiles.div_ceil(threads_per_block);
...
pub fn conv2d_direct(
    ...
    CudaConv2dDirect::conv2d_forward(input, weight, bias, (stride, stride), (padding, padding), 1)
}
