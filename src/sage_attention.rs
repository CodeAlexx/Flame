...
// Legacy SAGE attention module; to be revisited in later phase.
...
impl Default for SageAttention {
    fn default() -> Self {
        Self::new()
    }
}
...
impl SageAttention {
    ...
    pub fn forward(
        ...
        let (key_quantized, key_scales, value_quantized, value_scales) = if self.quantize_kv {
            ...
            // Skip quantization
            (
                key_smoothed.clone_result()?,
                Tensor::ones(
                    Shape::from_dims(&[batch_size, num_heads, seq_len_k, 1]),
                    key.device.clone(),
                )?,
                value_smoothed.clone_result()?,
                Tensor::ones(
                    Shape::from_dims(&[batch_size, num_heads, seq_len_k, 1]),
                    value.device.clone(),
                )?,
    ...
    fn apply_smoothing(&self, key: &Tensor, value: &Tensor) -> Result<(Tensor, Tensor)> {
        ...
        // Smooth factors
        let key_smooth = key_abs_mean
            .mul_scalar(self.smoothing_factor)?
            .add_scalar(1.0)?;
        let value_smooth = value_abs_mean
            .mul_scalar(self.smoothing_factor)?
            .add_scalar(1.0)?;
    ...
    fn quantize_kv_tensors(
        ...
        let f = device
            .get_func("sage_quantize_tensor", "sage_quantize_tensor")
            .ok_or_else(|| Error::Cuda("Failed to get quantize kernel".into()))?;
        ...
        // Quantize value
        let value_quantized =
            Tensor::zeros_dtype(value.shape().clone(), DType::I8, device.clone())?;
    ...
    fn matmul_q_k8bit(
        ...
        let f = device
            .get_func("sage_qk_matmul", "sage_qk_matmul")
            .ok_or_else(|| Error::Cuda("Failed to get qk matmul kernel".into()))?;
        ...
        let total = batch_size * num_heads * seq_len_q * seq_len_k;
        let num_blocks = total.div_ceil(threads_per_block);
        let num_blocks = u32::try_from(num_blocks)
            .map_err(|_| Error::InvalidOperation("sage_qk_matmul launch grid too large".into()))?;
    ...
    fn matmul_attn_v8bit(
        ...
        let f = device
            .get_func("sage_attn_v_matmul", "sage_attn_v_matmul")
            .ok_or_else(|| Error::Cuda("Failed to get attn-v matmul kernel".into()))?;
        ...
        // Launch kernel
        let threads_per_block = 256usize;
        let total = batch_size * num_heads * seq_len_q * head_dim;
        let num_blocks = total.div_ceil(threads_per_block);
        let num_blocks = u32::try_from(num_blocks).map_err(|_| {
            Error::InvalidOperation("sage_attn_v_matmul launch grid too large".into())
        })?;
    ...
    fn apply_causal_mask(
        ...
        let mask = Tensor::zeros_dtype(
            Shape::from_dims(&[seq_len_q, seq_len_k]),
            DType::F32,
            device,
        )?;
...
"#;
        ...
        let f = mask
            .device
            .get_func("causal_mask", "causal_mask")
            .ok_or_else(|| Error::Cuda("Failed to get causal mask kernel".into()))?;
