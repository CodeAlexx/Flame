#![allow(unused_variables, unused_imports, unused_doc_comments, dead_code)]
// Legacy checkpointing utilities; modern API to follow in Phase 3.
...
enum CheckpointedTensor {
    /// Tensor materialised on device (placeholder for future CPU offload)
    OnDevice(Tensor),
...
impl CheckpointManager {
    ...
    pub fn fetch_saved(
        ...
        if let Some(entry) = self.saved_activations.get(&id) {
            match entry {
                CheckpointedTensor::OnDevice(stored) => {
                    return Ok(Some(stored.clone_result()?));
                }
    ...
    pub fn register_tensor(&mut self, tensor: &Arc<Tensor>) {
        self.tensor_registry
            .insert(tensor.id, Arc::downgrade(tensor));
    }
    ...
    pub fn checkpoint_tensor(
        ...
        let tensor = self
            ...
            .ok_or_else(|| Error::InvalidOperation("Tensor not found in registry".into()))?;

        match self.policy {
            CheckpointPolicy::CPUOffload => {
                self.saved_activations.insert(
                    tensor_id,
                    CheckpointedTensor::OnDevice(tensor.clone_result()?),
                );
            }
            ...
            CheckpointPolicy::Adaptive {
                ...
                if self.memory_saved < memory_threshold && prefer_recompute {
                    ...
                    // Use on-device clone instead of CPU offload to keep GPU-only invariant
                    self.saved_activations.insert(
                        tensor_id,
                        CheckpointedTensor::OnDevice(tensor.clone_result()?),
                    );

                    self.memory_saved += tensor.shape().elem_count() * 2;
                }
            }
        }
    ...
    pub fn restore_tensor(&mut self, tensor_id: TensorId) -> Result<Tensor> {
        let checkpointed = self
            .saved_activations
            .remove(&tensor_id)
            .ok_or_else(|| Error::InvalidOperation("Checkpointed tensor not found".into()))?;

        let device = self
            .device
            .as_ref()
            .ok_or_else(|| Error::InvalidOperation("Device not set".into()))?;

        match checkpointed {
            CheckpointedTensor::OnDevice(stored) => stored.clone_result(),
    ...
    F: Fn(&[Tensor]) -> Result<Vec<Tensor>> + Send + Sync + Clone + 'static,
{
    ...
    pub fn forward(&self, inputs: &[Tensor]) -> Result<Vec<Tensor>> {
        ...
        let input_info: Vec<(Shape, Arc<CudaDevice>)> = inputs
            .iter()
