...
use flame_core::{bf16_elementwise, global_cuda_device, ops::add_iter::add_bf16_iter, DType, Result, Shape, Tensor};
...
fn bench_contig_reference(rows: usize, cols: usize) -> Result<()> {
    ...
    for _ in 0..10 {
        let _ = black_box(bf16_elementwise::add_bf16(&a, &b)?);
    }
    ...
    println!(
        "bf16_elementwise::add_bf16 [{rows}, {cols}] contig  {us:.2} µs/iter  ({:.2} ms / {} iters)",
