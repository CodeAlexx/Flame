...
fn bench(name: &str, n: usize, c: usize, h: usize, w: usize, groups: i32) -> Result<()> {
    ...
    for _ in 0..30 {
        let _ = black_box(cuda_ops_bf16::group_norm_bf16(&x, Some(&g), Some(&b), groups, 1e-5)?);
    }
    ...
    println!("  {:<24} N={} C={} H×W={}×{} groups={}  → {:>7.2} us/iter", name, n, c, h, w, groups, per_us);
    Ok(())
}
...
fn main() -> Result<()> {
    ...
    println!("=== group_norm fwd stats — mode: {} ===",
