...
fn run_one(name: &str, shape: &[usize; 4], in_ch: usize, out_ch: usize, k: usize, stride: usize, pad: usize) -> Result<()> {
    ...
    for _ in 0..30 { let _ = black_box(conv.forward(&x)?); }
    dev.synchronize().map_err(|e| FlameError::Cuda(format!("sync {e:?}")))?;
    let t0 = Instant::now();
    for _ in 0..ITERS { let _ = black_box(conv.forward(&x)?); }
    dev.synchronize().map_err(|e| FlameError::Cuda(format!("sync {e:?}")))?;
    let per_us = t0.elapsed().as_secs_f64() * 1e6 / ITERS as f64;
    println!("  {:<20} dims={:?}  k={} → {:>8.2} us/iter", name, shape, k, per_us);
    Ok(())
}
...
fn main() -> Result<()> {
    ...
    run_one("vae_up 128ch 1024²",   &[1, 128, 1024, 1024], 128, 128, 3, 1, 1)?;
