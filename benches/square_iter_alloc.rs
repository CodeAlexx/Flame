//! Microbench: contig-path overhead of `ops::square_iter` dispatcher,
//! plus strided-path reference.

#![cfg(feature = "cuda")]

use flame_core::{bf16_ops, global_cuda_device, tensor_iterator::ops::unary::square_bf16_iter, DType, Result, Shape, Tensor};
...
fn bench_contig_reference(rows: usize, cols: usize) -> Result<()> {
    ...
    for _ in 0..10 {
        let _ = black_box(bf16_ops::square_bf16(&x)?);
    }
    ...
    println!(
        "bf16_ops::square_bf16      [{rows}, {cols}] contig  {us:.2} µs/iter  ({:.2} ms / {} iters)",
...
fn bench_contig_through_iter(rows: usize, cols: usize) -> Result<()> {
    ...
    for _ in 0..10 {
        let _ = black_box(square_bf16_iter(&x)?);
    }
    dev.synchronize().map_err(|e| flame_core::Error::Cuda(format!("sync {e:?}")))?;
    ...
    let us = dt.as_nanos() as f64 / ITERS as f64 / 1_000.0;
    println!(
        "square_bf16_iter           [{rows}, {cols}] contig  {us:.2} µs/iter  ({:.2} ms / {} iters)",
        dt.as_secs_f64() * 1000.0,
        ITERS
    );
    Ok(())
...
fn bench_strided_via_iter(rows: usize, cols: usize) -> Result<()> {
    ...
    for _ in 0..10 {
        let _ = black_box(square_bf16_iter(&view)?);
    }
    dev.synchronize().map_err(|e| flame_core::Error::Cuda(format!("sync {e:?}")))?;
    ...
    let us = dt.as_nanos() as f64 / ITERS as f64 / 1_000.0;
    println!(
        "square_bf16_iter (strided) [{cols}, {rows}] T-view  {us:.2} µs/iter  ({:.2} ms / {} iters)",
        dt.as_secs_f64() * 1000.0,
        ITERS
    );
    Ok(())
...
fn main() -> Result<()> {
    ...
    println!("square_iter_alloc — {} iterations per case\n", ITERS);
