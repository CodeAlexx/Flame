...
use flame_core::{bf16_ops::swiglu_fused_bf16, global_cuda_device, DType, FlameError, Result, Shape, Tensor};
...
fn bench(name: &str, dims: &[usize]) -> Result<()> {
    ...
    println!("  {:<28} dims={:?} elems={:<10} → {:>7.2} us  ({:>5.0} MB/s rd+wr+rd)", name, dims, elems, per_us, mb_s);
    Ok(())
}
