...
use flame_core::{cuda_ops_bf16::slice_axis_bf16, global_cuda_device, DType, FlameError, Result, Shape, Tensor};
...
fn bench_slice(name: &str, dims: &[usize], axis: usize, start: usize, len: usize) -> Result<()> {
    ...
    let copied: usize = len * dims.iter().enumerate().filter(|(i, _)| *i != axis).map(|(_, &d)| d).product::<usize>();
    ...
    println!(
        ...
        name, dims, axis, start, start + len, copied, per_us, mb_total
    );
    Ok(())
}
...
fn main() -> Result<()> {
    let legacy = std::env::var("FLAME_SLICE_COPY_LEGACY")
        .map(|v| v != "0")
        .unwrap_or(false);
    let mode = if legacy { "LEGACY (slice_copy_kernel — div/mod per element)" }
               else      { "FAST PATH (cudaMemcpyAsync)" };
